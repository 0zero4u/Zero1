...
HIERARCHICAL REWARD UPDATE: The Optuna objective function has been re-engineered to use a
hierarchical reward structure. This embeds domain knowledge directly into the hyperparameter
search, prioritizing survival over profit, and profit over trading activity.
    ...
    def _on_step(self) -> bool:
        ...
        try:
            if self.n_calls % 700 == 0 and self.locals.get('infos'):
                ...
                # --- START OF FIX: Add thrashing diagnostics ---
...
class EnhancedFixedTrainer:
    ...
    def _make_env(self, rank: int, seed: int = 0, trial_params: Optional[Dict] = None) -> Callable:
        ...
        def _init():
            ...
            # Configure a unique logger for this worker to capture any exceptions.
            ...
            # Prevent logs from propagating to the root logger in the main process
            ...
            try:
                ...
                # --- START OF DIAGNOSTIC FIX ---
                # Log the exception to the worker's dedicated file before crashing.
    ...
    def objective(self, trial) -> float:
        """
        MODIFIED: Optuna objective with a HIERARCHICAL reward structure.
        This implements the principled search where survival > profit > efficiency.
        """
        ...
        try:
            # ==============================================================================
            # START OF HIERARCHICAL REWARD WEIGHT DEFINITION
            # ==============================================================================

            # --- Step 1: Anchor the hierarchy to the core profit signal ---
            reward_scaling_factor = trial.suggest_float('reward_scaling_factor', 150.0, 350.0)
            reward_weight_base_return = trial.suggest_float('reward_weight_base_return', 4.0, 6.0)

            # Calculate the reward for a benchmark "good trade" (0.1% profit) to anchor penalties.
            good_trade_profit_pct = 0.001
            scaled_return = good_trade_profit_pct * reward_scaling_factor
            normalized_return = np.tanh(scaled_return * 0.5)
            good_trade_reward = normalized_return * reward_weight_base_return
            
            # --- Step 2: Set the Efficiency Hurdle ---
            # The penalty for acting should be a high fraction of the reward from a single good trade.
            freq_penalty_ratio = trial.suggest_float('freq_penalty_ratio', 0.85, 0.98)
            reward_weight_frequency_penalty = -good_trade_reward * freq_penalty_ratio

            # --- Step 3: Set the Survival Veto ---
            # The penalty for a drawdown must be a powerful veto, costing several "good trades".
            drawdown_veto_factor = trial.suggest_float('drawdown_veto_factor', 2.0, 5.0)
            reward_weight_drawdown_penalty = -good_trade_reward * drawdown_veto_factor
            
            # ==============================================================================
            # END OF HIERARCHICAL REWARD WEIGHT DEFINITION
            # ==============================================================================
            ...
            trial_params = {
                ...
                'transformer_d_model': trial.suggest_categorical('transformer_d_model', [64, 96]),
                'transformer_n_heads': trial.suggest_categorical('transformer_n_heads', [4, 6, 8]),
                'transformer_dim_feedforward': trial.suggest_categorical('transformer_dim_feedforward', [256, 512]),
                'transformer_num_layers': trial.suggest_int('transformer_num_layers', 1, 2),
                ...
                # Strategy & Risk
                'leverage': trial.suggest_float('leverage', 6.0, 15.0),
                'max_margin_allocation_pct': trial.suggest_float('max_margin_allocation_pct', 0.02, 0.06),
                
                # --- Reward Weights ---
                'reward_scaling_factor': reward_scaling_factor,
                'reward_weight_base_return': reward_weight_base_return,
                # Use our hierarchically defined weights
                'reward_weight_frequency_penalty': reward_weight_frequency_penalty,
                'reward_weight_drawdown_penalty': reward_weight_drawdown_penalty,
                
                # --- Step 4: Tune Secondary Nudges within safe, bounded ranges ---
                'reward_weight_transaction_penalty': trial.suggest_float('reward_weight_transaction_penalty', -2.0, -0.5),
                'reward_weight_position_penalty': trial.suggest_float('reward_weight_position_penalty', -0.15, -0.03),
                'reward_weight_exploration_bonus': trial.suggest_float('reward_weight_exploration_bonus', 0.01, 0.05),
                ...
                'inactivity_grace_period': trial.suggest_int('inactivity_grace_period', 12, 30),
                'penalty_ramp_up_steps': trial.suggest_int('penalty_ramp_up_steps', 20, 60),
            }
    ...
    try:
        ...
        if optimization_trials > 0 and OPTUNA_AVAILABLE:
            ...
            best_params = {
                ...
                'reward_weight_transaction_penalty': -1.0, # Adjusted based on hierarchy
                'reward_weight_drawdown_penalty': -9.0, # Adjusted based on hierarchy
                'reward_weight_position_penalty': -0.1,
                'reward_weight_exploration_bonus': 0.03, # Kept low
                'reward_weight_inactivity_penalty': -0.8,
                'reward_weight_frequency_penalty': -4.0, # Adjusted based on hierarchy
