...
class HierarchicalTradingEnvironment(gym.Env):
    ...
    def __init__(self, df_base_ohlc: pd.DataFrame, normalizer: Normalizer, config=None):
        ...
        try:
            ...
            for freq in all_required_freqs:
                if freq in ['CONTEXT', 'PORTFOLIO_STATE']: continue
            ...
            for key, lookback in self.strat_cfg.lookback_periods.items():
                ...
                elif key == FeatureKeys.PORTFOLIO_STATE: shape = (seq_len, lookback)
    ...
    def _initialize_stateful_features(self):
        """Create instances of all stateful calculators and their history deques."""
        logger.info("Initializing stateful feature calculators...")
        ...
        self.calculator_definitions = {
            'bbw_1h_pct': {'class': StatefulBBWPercentRank, 'params': {'period': 20, 'rank_window': 250}, 'timeframe': '1H', 'source_col': 'close'},
            'price_dist_ma_4h': {'class': StatefulPriceDistanceMA, 'params': {'period': 50}, 'timeframe': '4H', 'source_col': 'close'},
            'sr_3m': {'class': StatefulSRDistances, 'params': {'period': sr_window, 'num_levels': 1}, 'timeframe': '3T', 'source_col': 'close'},
            'sr_15m': {'class': StatefulSRDistances, 'params': {'period': sr_window, 'num_levels': 2}, 'timeframe': '15T', 'source_col': 'close'},
            'sr_1h': {'class': StatefulSRDistances, 'params': {'period': sr_window, 'num_levels': 2}, 'timeframe': '1H', 'source_col': 'close'},
        }

        for key, definition in self.calculator_definitions.items():
            self.feature_calculators[key] = definition['class'](**definition['params'])
            self.last_update_timestamps[definition['timeframe']] = pd.Timestamp(0, tz='UTC')
    ...
    def _update_stateful_features(self, step_index: int):
        ...
        for calc_key, definition in self.calculator_definitions.items():
            ...
            if latest_bar_timestamp > self.last_update_timestamps[timeframe]:
                ...
                new_data_point = df_tf[definition['source_col']].iloc[latest_bar_idx]
    ...
    def _get_single_step_observation(self, step_index) -> dict:
        ...
        try:
            ...
            for key_enum, lookback in self.strat_cfg.lookback_periods.items():
                key = key_enum.value
                if key == FeatureKeys.PORTFOLIO_STATE.value: obs[key] = np.zeros(lookback, dtype=np.float32)
                elif key == 'context': obs[key] = np.zeros(lookback, dtype=np.float32)
...
def process_trades_for_period(period_name: str, force_reprocess: bool = False):
    from processor import EnhancedDataProcessor
    return EnhancedDataProcessor().process_raw_trades_parallel(period_name, force_reprocess)