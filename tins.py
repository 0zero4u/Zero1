...
class EnhancedHierarchicalAttentionFeatureExtractor(BaseFeaturesExtractor):
    ...
    def __init__(self, observation_space: spaces.Dict, arch_cfg=None):
        try:
            ...
            expert_input_dims = {'flow': 0, 'volatility': 0, 'value_trend': 0, 'context': 0}
            ...
            # Context feature dimension is from lookback_periods
            expert_input_dims['context'] = self.cfg.lookback_periods[SETTINGS.strategy.lookback_periods.CONTEXT]
            ...
            # --- Attention and Projection Layers (remain the same) ---
            total_expert_features = 4 * expert_hidden_size
            ...
            self.output_projection = nn.Linear(expert_hidden_size, self.features_dim)
    ...
    def forward(self, observations: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        try:
            ...
            # --- Dynamically process signals and group them for experts ---
            expert_inputs = {
            ...
            # Context input is directly from observations
...
if __name__ == "__main__":
    ...
    try:
        ...
        for key, lookback in s_cfg.lookback_periods.items():
            ...
            else: # price, volume_delta, context
