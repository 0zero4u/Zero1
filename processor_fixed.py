...
FIXED: Updated deprecated pandas frequency aliases.
"""
...
# REFINED: Import the centralized calculator map
...
class EnhancedDataProcessor:
    ...
    # FIXED: Updated frequency alias mapping function
    def _normalize_frequency_alias(self, freq: str) -> str:
        """Convert deprecated frequency aliases to new ones."""
        freq_mapping = {
            'T': 'min',     # Minutes
            'H': 'h',       # Hours  
            'S': 's',       # Seconds
        ...
        # Handle patterns like '1T', '15T', '1H', '4H', '20S'
        for old, new in freq_mapping.items():
            if freq.endswith(old):
                return freq.replace(old, new)
        
        return freq  # Return as-is if no mapping needed
    ...
    def _process_single_file(self, file_info: Dict) -> Optional[str]:
        """Process a single file with a memory-efficient streaming approach."""
        raw_path = file_info['raw_path']
        output_path = file_info['output_path']
        filename = os.path.basename(raw_path)
        
        # The existence of the output file itself acts as our cache.
        # The original memory-intensive caching logic has been removed.
                ...
                with z.open(csv_filename) as f:
                    ...
                    try:
                        ...
                        with pq.ParquetWriter(output_path, table.schema, compression='snappy') as writer:
                            ...
                            # Process remaining chunks in a streaming fashion
                            total_chunks = 1
    ...
    def _create_bars_with_dask(self, all_trade_files: List[str], additional_features: bool) -> pd.DataFrame:
        ...
        try:
            ...
            # FIXED: Use normalized resample frequency
            resample_freq = self._normalize_frequency_alias(self.cfg.base_bar_timeframe.value)
    ...
    def _create_bars_with_pandas(self, all_trade_files: List[str], additional_features: bool) -> pd.DataFrame:
        """Create bars using pandas with a memory-efficient, file-by-file, chunk-by-chunk approach."""
        logger.info(f"Loading {len(all_trade_files)} trade files with pandas (fully optimized streaming mode)...")
        
        try:
            final_bars_df = pd.DataFrame()
            
            # FIXED: Use normalized resample frequency
            resample_freq = self._normalize_frequency_alias(self.cfg.base_bar_timeframe.value)
            
            # Define the aggregation rules once
            agg_rules = {
                'price': ['first', 'max', 'min', 'last', 'count'],
                'size': 'sum',
                'price_volume': 'sum',
            ...
            # Define the aggregation rules for re-grouping
            regroup_rules = {
                ('price', 'first'): 'first', ('price', 'max'): 'max',
                ('price', 'min'): 'min', ('price', 'last'): 'last',
                ('price', 'count'): 'sum', ('size', 'sum'): 'sum',
                ('price_volume', 'sum'): 'sum', ('buy_volume', 'sum'): 'sum',
                ('sell_volume', 'sum'): 'sum'
            }
            
            for file_path in tqdm(all_trade_files, desc="Creating bars from files"):
                # NEW: Open the Parquet file without loading it all into memory
                pf = pq.ParquetFile(file_path)
                
                # NEW: Iterate over chunks (row groups) within the single Parquet file
                for batch in pf.iter_batches(batch_size=2_000_000):
                    ...
                    # Perform operations on the small chunk
                    ...
                    bars_chunk = df.resample(resample_freq).agg(agg_rules)
                    
                    # Incrementally combine and re-aggregate to keep memory low
                    combined = pd.concat([final_bars_df, bars_chunk])
                    final_bars_df = combined.groupby(level=0).agg(regroup_rules)
            
            if final_bars_df.empty:
                logger.warning("No bar data was generated from any files.")
                return pd.DataFrame()
            
            # Flatten MultiIndex columns and rename
            final_bars_df.columns = [
                'open', 'high', 'low', 'close', 'trade_count', 'volume',
                'price_volume_sum', 'buy_volume_sum', 'sell_volume_sum'
            ]
            
            # Calculate VWAP and Volume Delta
            final_bars_df['vwap'] = final_bars_df['price_volume_sum'] / (final_bars_df['volume'] + 1e-9)
            final_bars_df['volume_delta'] = final_bars_df['buy_volume_sum'] - final_bars_df['sell_volume_sum']
            final_bars_df = final_bars_df.drop(columns=['price_volume_sum', 'buy_volume_sum', 'sell_volume_sum'])
            
            if additional_features:
                final_bars_df = self._add_enhanced_features(final_bars_df)
            
            final_bars_df = final_bars_df[final_bars_df['trade_count'] > 0]
...
def generate_stateful_features_for_fitting(bars_df: pd.DataFrame, strat_cfg: StrategyConfig) -> pd.DataFrame:
    ...
    FIXED: Updated frequency alias normalization.
    """
    ...
    # FIXED: Normalize frequency aliases before resampling
    processor = EnhancedDataProcessor()  # Create instance to access the method
    required_freqs = {processor._normalize_frequency_alias(calc.timeframe) for calc in strat_cfg.stateful_calculators}
    ...
    # Calculate each feature by calling its centralized vectorized method
    for calc_cfg in tqdm(strat_cfg.stateful_calculators, desc="Calculating features"):
        ...
        # Prepare arguments for the vectorized method
        ...
        if calc_cfg.class_name == 'StatefulVWAPDistance':
            ...
            # FIXED: Use normalized frequency
            normalized_freq = processor._normalize_frequency_alias(calc_cfg.timeframe)
            df_tf = timeframes[normalized_freq]
        ...
        # --- START OF FIX ---
        # The original logic failed to map generic output columns (e.g., 'dist_s1')
        # to the specific, timeframe-aware keys defined in the config (e.g., 'dist_s1_3m').
        # This new logic assumes the order of columns from the calculator matches the
        # order of output_keys in the config and renames them accordingly.
        if len(result_df.columns) != len(calc_cfg.output_keys):
            logger.warning(
                f"Mismatched output count for {calc_cfg.class_name}. "
                f"Expected {len(calc_cfg.output_keys)}, but got {len(result_df.columns)}. Skipping."
            )
            continue
        
        # Directly assign the config's output keys as the new column names.
        # This is robust and relies on the calculator preserving the output order.
        result_df.columns = calc_cfg.output_keys
        
        # Reindex and merge each valid, renamed column into the main features DataFrame
