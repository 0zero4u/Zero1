        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            for _ in range(self.n_eval_episodes):
                ...
                while not done:
                    ...
                    # --- START OF FIX: Correct unpacking for VecEnv ---
                    # A VecEnv's step() returns 4 items: observations, rewards, dones, and infos.
                    # The original code was trying to unpack 5 items, causing the crash.
                    obs, rewards, dones, infos = self.eval_env.step(action)
                    # `dones` is a boolean array. For a single-environment VecEnv, we take the first element.
                    done = dones[0]
...
class EnhancedFixedTrainer:
    ...
    def objective(self, trial) -> float:
        ...
        vec_env, eval_env = None, None # Ensure they are defined for the finally block
        try:
            ...
            # --- START OF FIX: ADDED DETAILED LOGGING ---
            log_message = f"TRIAL {trial.number} FAILED with error: {e}"
            if "not enough values to unpack" in str(e):
                log_message += (
                    "\n>>> DIAGNOSIS: This is a classic issue where a callback expects a single environment's "
                    "step() return (5 values: obs, reward, terminated, truncated, info) but receives a "
                    "vectorized environment's step() return (4 values: obs, rewards, dones, infos). "
                    "The `EvalCallback` implementation has been corrected for this."
                )
