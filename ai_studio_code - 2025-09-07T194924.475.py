...
# FIXED: Import the new fixed engine
from engine import FixedHierarchicalTradingEnvironment
...
class FixedTrainer:
    """
    FIXED: Trainer that prevents turtling and promotes learning.
    """
    ...
    def __init__(self, base_config: Optional[Dict] = None, use_wandb: bool = False):
        ...
        try:
            logger.info("🔄 Pre-loading training data for Fixed Reward System...")
        ...
        def _init():
            try:
                ...
                if trial_params:
                    ...
                    # FIXED: Better balanced reward weights
                    reward_weights = {
                        'base_return': trial_params.get('reward_weight_base_return', 3.0),
                        'risk_adjusted': trial_params.get('reward_weight_risk_adjusted', 0.4),
                        ...
                        'risk_bonus': trial_params.get('reward_weight_risk_bonus', 0.3),
                        'exploration_bonus': trial_params.get('reward_weight_exploration_bonus', 0.5),
                        'inactivity_penalty': trial_params.get('reward_weight_inactivity_penalty', -0.2),
                        'action_reward': trial_params.get('reward_weight_action_reward', 0.3),
                    }
    ...
    def create_model(self, trial_params: Dict, vec_env: VecEnv) -> PPO:
        ...
        try:
            ...
            model = PPO(
                ...
                ent_coef=trial_params.get('ent_coef', 0.01),  # FIXED: Better entropy coefficient
            ...
            logger.info("✅ Created PPO model with fixed hyperparameters")
            return model
    ...
    def objective(self, trial) -> float:
        """FIXED: Optuna objective that promotes action-taking and learning."""
        ...
        try:
            # FIXED: Better hyperparameter ranges
            trial_params = {
                # FIXED: Higher learning rate range
                ...
                # FIXED: Better entropy coefficient range to prevent collapse
                ...
                # FIXED: Better reward scaling factor range
                'reward_scaling_factor': trial.suggest_float('reward_scaling_factor', 50.0, 200.0),

                # FIXED: Better reward weight ranges to prevent dominance
                'reward_weight_base_return': trial.suggest_float('reward_weight_base_return', 2.0, 5.0),
                'reward_weight_risk_adjusted': trial.suggest_float('reward_weight_risk_adjusted', 0.1, 0.8),
                'reward_weight_stability': trial.suggest_float('reward_weight_stability', 0.1, 0.5),
                ...
                'reward_weight_risk_bonus': trial.suggest_float('reward_weight_risk_bonus', 0.1, 0.5),
                'reward_weight_exploration_bonus': trial.suggest_float('reward_weight_exploration_bonus', 0.2, 1.0),
                'reward_weight_inactivity_penalty': trial.suggest_float('reward_weight_inactivity_penalty', -0.5, -0.05),
                'reward_weight_action_reward': trial.suggest_float('reward_weight_action_reward', 0.1, 0.8),
            ...
            if self.use_wandb:
                callbacks.append(WandbCallback("crypto_trading_fixed", experiment_name, trial_params))
            ...
            # FIXED: Better performance metric that considers action-taking
            if perf_callback.portfolio_values:
                ...
                # FIXED: More balanced objective function
    ...
    def optimize(self, n_trials: int = 50, timeout: Optional[int] = None):
        """Run optimization with the Fixed Reward System."""
        if not OPTUNA_AVAILABLE:
        ...
        try:
            ...
            logger.info(f"🚀 Starting optimization for Fixed Reward System (Anti-Turtling)")
            ...
            logger.info("🎯 Fixed System Optimization Results:")
    ...
    def train_best_model(self, best_params: Dict, final_training_steps: int) -> PPO:
        ...
        try:
            logger.info("🎯 Training final model with fixed parameters...")
...
def train_model_fixed(optimization_trials: int = 20,
    ...
    """
    FIXED: Advanced training that prevents turtling and encourages learning.
    Returns the path to the trained model.
    """
    try:
        logger.info("🎯 Starting FIXED Training (Anti-Turtling System)")
        logger.info("✅ FIXES APPLIED: Balanced rewards, higher LR, better exploration")

        trainer = FixedTrainer(use_wandb=use_wandb)
        ...
        if optimization_trials > 0 and OPTUNA_AVAILABLE:
            ...
            logger.info("Skipping optimization, using fixed default parameters.")
            # FIXED: Better default parameters
            best_params = {
                'learning_rate': 5e-4,  # FIXED: Higher learning rate
                'n_steps': 2048, 
                'batch_size': 64, 
                'n_epochs': 10,
                'gamma': 0.99, 
                'gae_lambda': 0.95, 
                'clip_range': 0.2, 
                'ent_coef': 0.01,  # FIXED: Better entropy coefficient
                'max_grad_norm': 0.5, 
                ...
                'reward_scaling_factor': 100.0,  # FIXED: Better scaling
                
                # FIXED: Balanced reward weights
                'reward_weight_base_return': 3.0,
                'reward_weight_risk_adjusted': 0.4,
                ...
                'reward_weight_risk_bonus': 0.3,
                'reward_weight_exploration_bonus': 0.5,
                'reward_weight_inactivity_penalty': -0.2,
                'reward_weight_action_reward': 0.3,
        ...
        logger.info("✅ Anti-turtling measures applied")
        logger.info("✅ Reward balance restored")
        logger.info("✅ Learning rate fixed")
...
if __name__ == "__main__":
    ...
    try:
        # NOTE: The function name is corrected here as well for direct execution
        ...
        logger.error(f"Fixed training example failed: {e}")
        raise