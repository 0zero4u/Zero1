# UPDATED: config.py with Transformer Architecture Support
...
# --- UPDATED: TRANSFORMER ARCHITECTURE CONFIG ---

class ModelArchitectureConfig(BaseModel):
    """
    UPDATED: Configuration for the Transformer-based Neural Architecture
    
    REMOVED: LSTM-specific parameters
    ADDED: Transformer architecture parameters
    RENAMED: expert_lstm_hidden_size -> expert_output_dim
    """
    
    # NEW: Transformer Architecture Parameters
    ...
    # RENAMED: More accurate parameter names
    expert_output_dim: int = Field(default=32, ge=8, le=256, description="Expert head output dimension (renamed from expert_lstm_hidden_size)")
    ...
    @validator('transformer_n_heads')
    def validate_transformer_n_heads(cls, v, values):
        """Validate attention heads count."""
        if v < 1:
            raise ValueError("Number of attention heads must be at least 1")
        if v > 16:
            warnings.warn(f"Large number of attention heads ({v}) may impact performance")
        return v

    @validator('transformer_dim_feedforward')
    def validate_transformer_dim_feedforward(cls, v, values):
        """Validate feed-forward dimension."""
        if 'transformer_d_model' in values:
            d_model = values['transformer_d_model']
            if v < d_model:
                warnings.warn(f"Feed-forward dim ({v}) smaller than d_model ({d_model}), may limit capacity")
        return v

    @validator('attention_head_features')
    def validate_attention_features(cls, v, values):
        """Validate attention head features."""
        if 'expert_output_dim' in values and v < values['expert_output_dim']:
            warnings.warn(f"Attention features ({v}) smaller than expert output dim ({values['expert_output_dim']})")
        return v
...
class StrategyConfig(BaseModel):
    ...
    # Configurable Reward Horizon System
    ...
    # Define precomputed features from processor.py as a single source of truth.
    ...
    # Declarative configuration for stateful feature calculators
    ...
    # UPDATED: Model architecture reference (now Transformer-based)
...
class PPOTrainingConfig(BaseModel):
    ...
    model_output_file: str = "ppo_hierarchical_attention_transformer.zip"  # Updated filename
...
class GlobalConfig(BaseModel):
    """Enhanced global configuration with Transformer architecture and reward horizon support."""
...
def create_config(env: Optional[Environment] = None, **overrides) -> GlobalConfig:
    """
    ENHANCED: Create configuration with environment-specific settings, leverage, reward horizon, and Transformer support.
    """
    try:
...
def validate_configuration(config: GlobalConfig) -> List[str]:
    """
    ENHANCED: Validate configuration with leverage, reward horizon, and Transformer architecture awareness.
    """
    warnings_list = []
    ...
    try:
        ...
        # Enhanced strategy validation with leverage awareness
        ...
        # NEW: Transformer architecture validation
        arch = config.strategy.architecture
        
        # Validate d_model divisibility by n_heads
        if arch.transformer_d_model % arch.transformer_n_heads != 0:
            warnings_list.append(f"Transformer d_model ({arch.transformer_d_model}) not divisible by n_heads ({arch.transformer_n_heads})")

        # Check for reasonable architecture sizes
...
def validate_and_fix_configuration(config: GlobalConfig) -> Tuple[GlobalConfig, List[str]]:
    """
    ENHANCED: Validate configuration and automatically fix common issues with Transformer architecture awareness.
    """
    warnings_list = []
    ...
    try:
        ...
        # NEW: Auto-fix Transformer architecture issues
        arch = config.strategy.architecture
        
        # Fix d_model divisibility by n_heads
        if arch.transformer_d_model % arch.transformer_n_heads != 0:
            old_d_model = arch.transformer_d_model
            arch.transformer_d_model = ((arch.transformer_d_model // arch.transformer_n_heads) + 1) * arch.transformer_n_heads
            warnings_list.append(f"Auto-adjusted transformer_d_model for divisibility: {old_d_model} -> {arch.transformer_d_model}")
...
if __name__ == "__main__":
    print("--- Loading ENHANCED Configuration with Transformer Architecture ---")
    ...
    try:
        ...
        # NEW: Display Transformer architecture configuration
        arch = SETTINGS.strategy.architecture
        print(f"\n--- Transformer Architecture Configuration ---")
        print(f"D_Model: {arch.transformer_d_model}")
        print(f"N_Heads: {arch.transformer_n_heads}")
        print(f"Dim_Feedforward: {arch.transformer_dim_feedforward}")
        print(f"Num_Layers: {arch.transformer_num_layers}")
        print(f"Expert_Output_Dim: {arch.expert_output_dim}")
        print(f"Attention_Head_Features: {arch.attention_head_features}")
        print(f"Dropout_Rate: {arch.dropout_rate}")
        ...
        print("\n✅ ENHANCED Configuration with Transformer architecture loading and validation complete.")
        ...
        reward_scaling_factor = 200.0 / SETTINGS.strategy.leverage
        print(f"\nReward Scaling Factor (for reference): {reward_scaling_factor:.2f}")

        # NEW: Calculate approximate model complexity
        arch_complexity = arch.transformer_d_model * arch.transformer_n_heads * arch.transformer_num_layers
        print(f"Transformer Complexity Score: {arch_complexity}")
