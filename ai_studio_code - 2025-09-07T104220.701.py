# REFINED: engine.py with Path-Aware Decayed Reward System (CORRECTED & FIXED)
...
CORRECTION: Fixed critical bugs in the reward calculator where penalties were
incorrectly inverted into rewards due to a "double negative" error.

FIX APPLIED: Implemented solutions from agent_issues_fix.md to create a balanced
reward signal, fix the risk-adjusted calculation, reduce noise, and incentivize trading.
"""
...
class AdvancedRewardCalculator:
    ...
    def __init__(self, config, leverage: float = 10.0, reward_weights: Dict[str, float] = None):
        ...
        ## FIX: Adjusted reward weights to be less punitive and more balanced.
        self.weights = reward_weights or {
            'base_return': 2.0,          # Increased focus on returns
            'risk_adjusted': 0.1,        # Reduced impact of potentially noisy component
            'stability': 0.1,            # Reduced from 0.2
            'transaction_penalty': -0.02, # Reduced penalty to encourage exploration
            'drawdown_penalty': -0.2,    # Reduced penalty from -0.4
            'position_penalty': -0.02,   # Reduced penalty from -0.05
            'risk_bonus': 0.2,           # Increased positive incentive
            'exploration_bonus': 0.1     # NEW: Reward taking action
        }
        
        ## FIX: Reduced aggressive scaling factor to decrease noise and improve stability.
        self.scaling_factor = 100.0 / self.leverage # Was 200.0
    ...
    def calculate_enhanced_reward(self, prev_value: float, curr_value: float,
        ...
        try:
            ...
            ## FIX: Increased buffer size for more robust stats and fixed calculation.
            if len(self.return_buffer) >= 50: # Was 30
                ...
                ## FIX: Clip score to prevent extreme values from dominating the reward.
                risk_adjusted_score = np.clip(np.tanh(risk_adjusted_score * 5), -2.0, 2.0)
                components['risk_adjusted'] = risk_adjusted_score * self.weights['risk_adjusted']
            else:
                ## FIX: Default to neutral (0.0) when buffer is not full.
            ...
            ## NEW: Add an exploration bonus to incentivize trading activity.
            if position_change > 0.01:  # A small threshold to count as a meaningful action
    ...
    def calculate_path_aware_reward(self, portfolio_path: List[float], action: np.ndarray,
        ...
        returns over the horizon period, providing more stable and
        risk-aware learning signals.
...
class EnhancedHierarchicalTradingEnvironment(gym.Env):
        ...
        try:
            ...
            for freq in all_required_freqs:
                if freq not in self.timeframes:
                    # FIX: Expanded aggregation rules to include precomputed features
                    agg_rules = {
    ...
    def step(self, action: np.ndarray):
        ...
        try:
            ...
            # FIX: Align episode termination with configured max_drawdown_threshold
            # NOTE: Consider increasing strat_cfg.max_drawdown_threshold to 0.25 in config
            # to allow the agent more room to learn from mistakes.
    ...
    def _get_single_step_observation(self, step_index) -> dict:
        ...
        try:
            ...
            raw_obs[FeatureKeys.PORTFOLIO_STATE.value] = np.array([
                normalized_position, normalized_pql, margin_ratio, regime_encoding, vol_adjusted_signal
            ], dtype=np.float32)
