...
class EnhancedMACDCell(nn.Module):
    """IMPROVED: Implements the full MACD calculation using learnable EMA layers."""
    ...
    def forward(self, price_series: torch.Tensor) -> torch.Tensor:
        ...
        # 1. Create sliding windows for the slow EMA calculation.
        # This results in `signal_period` windows, each of size `slow_period`.
        prices_for_macd = price_series[:, -required_length:]
        slow_windows = prices_for_macd.unfold(dimension=1, size=self.slow_period, step=1)

        # 2. Slice fast windows from the end of the slow windows.
        fast_windows = slow_windows[:, :, -self.fast_period:]

        # 3. Apply the MA layers in a vectorized way. The linear layer operates on the last dim.
        slow_ema = self.slow_ma(slow_windows).squeeze(-1)
        fast_ema = self.fast_ma(fast_windows).squeeze(-1)
        # Shape of both slow_ema and fast_ema: (batch_size, signal_period)

        # 4. Calculate the MACD line for the last `signal_period` steps.
        macd_line = fast_ema - slow_ema
        # Shape: (batch_size, signal_period)

        # 5. Calculate the signal line using the MACD line history.
        signal_line = self.signal_ma(macd_line)
        # Shape: (batch_size, 1)

        # 6. Calculate the histogram from the most recent MACD value and the signal line.
...
class EnhancedRSICell(nn.Module):
    """IMPROVED: Implements the RSI calculation."""
...
class EnhancedATRCell(nn.Module):
    """IMPROVED: Implements the ATR calculation."""
...
class EnhancedBBandsCell(nn.Module):
    """IMPROVED: Implements a Bollinger Bands signal."""
...
# --- IMPROVED EXPERT HEAD WITH ATTENTION ---

class AttentionEnhancedExpertHead(nn.Module):
    """IMPROVED: Expert head with self-attention for sequence processing."""

    def __init__(self, input_dim: int, hidden_dim: int, dropout: float = 0.1):
        super().__init__()
        
        # Self-attention layer
        self.self_attention = MultiHeadSelfAttention(input_dim, n_heads=4, dropout=dropout)
...
class EnhancedHierarchicalAttentionFeatureExtractor(BaseFeaturesExtractor):
    """IMPROVED: Enhanced feature extractor with attention-based expert heads."""
    ...
    def __init__(self, observation_space: spaces.Dict, arch_cfg=None):
        ...
        print("--- Building Enhanced Hierarchical Attention Feature Extractor (IMPROVED VERSION) ---")
        ...
        # IMPROVED: Use attention-enhanced expert heads
        self.flow_head = AttentionEnhancedExpertHead(3, self.arch_cfg.expert_lstm_hidden_size)
        self.volatility_head = AttentionEnhancedExpertHead(3, self.arch_cfg.expert_lstm_hidden_size)
        self.value_trend_head = AttentionEnhancedExpertHead(4, self.arch_cfg.expert_lstm_hidden_size)
        self.context_head = AttentionEnhancedExpertHead(
            SETTINGS.strategy.lookback_periods['context'], 
    ...
    def _process_all_signals(self, observations: Dict[str, torch.Tensor],
        ...
        for cell_name, cell in self.cells.items():
            ...
            if prefix not in self.cell_data_map: 
                continue
