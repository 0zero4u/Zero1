# REFINED: config.py with Immediate Reward System

"""
REFINEMENT: Configuration simplified to support an immediate, step-by-step reward
system. All parameters related to the flawed path-aware, delayed reward horizon
have been removed for clarity and correctness.
"""
...
class StrategyConfig(BaseModel):
    """Simplified strategy configuration focusing on immediate rewards."""
    
    # NOTE: All path-aware reward horizon parameters have been removed.
    # The agent now learns from immediate, step-by-step rewards, which is
    # compatible with the PPO algorithm.
...
class GlobalConfig(BaseModel):
    ...
    def get_timeframe_seconds(self, timeframe: TimeFrameType | str) -> int:
        ...
        timeframe_to_seconds_map = {"20S": 20, "1T": 60, "3T": 180, "5T": 300, "15T": 900, "1H": 3600, "4H": 14400, "1D": 86400}
        ...
        try:
            if 'S' in timeframe_key: return int(timeframe_key.replace('S', ''))
            if 'T' in timeframe_key: return int(timeframe_key.replace('T', '')) * 60
            if 'H' in timeframe_key: return int(timeframe_key.replace('H', '')) * 3600
            if 'D' in timeframe_key: return int(timeframe_key.replace('D', '')) * 86400
...
def create_config(env: Optional[Environment] = None, **overrides) -> GlobalConfig:
    try:
        ...
        base_config = {
            Environment.DEVELOPMENT: {"training": {"total_timesteps": 50_000, "checkpoint_frequency": 1000}, "log_level": "DEBUG"},
            Environment.STAGING: {"training": {"total_timesteps": 200_000, "checkpoint_frequency": 5000}, "log_level": "INFO"},
            Environment.PRODUCTION: {"training": {"total_timesteps": 1_000_000, "checkpoint_frequency": 10000}, "log_level": "WARNING", "enable_wandb": True}
        }
        ...
        logger.info("Immediate reward system configured.")
...
if __name__ == "__main__":
    print("--- Loading SIMPLIFIED Configuration with Immediate Rewards ---")
    try:
        setup_environment()
        ...
        print("\n--- SIMPLIFIED: Immediate Reward System ---")
        print("NOTE: Path-aware reward horizon has been removed for PPO compatibility.")

        print(f"\n--- Declarative Feature Configuration ---")
        print("Context Features (declarative pattern):")
        ...
        print("\n✅ SIMPLIFIED configuration loading complete with immediate rewards.")
    except Exception as e:
