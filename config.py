...
2. REPLACED static scaling with a dynamic, weighted reward component system
3. Simplified reward configuration for immediate rewards
4. Preserved all other functionality (features, architecture, etc.)
5. FIXED deprecated pandas frequency aliases (T->min, H->h, S->s)
"""
...
class TimeFrameType(str, Enum):
    # FIXED: Updated to use non-deprecated frequency aliases
    SECOND_20 = "20s"      # was "20S"
    MINUTE_1 = "1min"      # was "1T"
    MINUTE_3 = "3min"      # was "3T"
    MINUTE_5 = "5min"      # was "5T"
    MINUTE_15 = "15min"    # was "15T"
    HOUR_1 = "1h"          # was "1H"
    HOUR_4 = "4h"          # was "4H"
    DAY_1 = "1D"
...
class PPOTrainingConfig(BaseModel):
    ...
    # --- START OF FIX: Update PPO defaults to discourage thrashing ---
    clip_range: float = Field(default=0.1, ge=0.05, le=0.5, description="PPO clip range")
    ent_coef: float = Field(
        default=0.01, ge=0.0, le=0.1,
        description="Entropy coefficient. Lower values encourage more confident actions."
    )
    # --- END OF FIX ---
    ...
    # FIXED: Replaced deprecated @validator with @model_validator for Pydantic V2
    @model_validator(mode='after')
    def validate_batch_size(self) -> 'PPOTrainingConfig':
        if self.batch_size > self.n_steps:
            raise ValueError(f"Batch size ({self.batch_size}) cannot be larger than n_steps ({self.n_steps})")
        return self
...
class GlobalConfig(BaseModel):
    ...
    in_sample_start: datetime = datetime(2025, 1, 1, tzinfo=timezone.utc)  # FIXED: Added timezone
    in_sample_end: datetime = datetime(2025, 5, 31, tzinfo=timezone.utc)   # FIXED: Added timezone
    out_of_sample_start: datetime = datetime(2025, 6, 1, tzinfo=timezone.utc)  # FIXED: Added timezone
    out_of_sample_end: datetime = datetime(2025, 7, 31, tzinfo=timezone.utc)   # FIXED: Added timezone
    ...
    def get_timeframe_seconds(self, timeframe: TimeFrameType | str) -> int:
        ...
        FIXED: Updated to handle new frequency aliases.
        """
        timeframe_str = timeframe.value if isinstance(timeframe, Enum) else str(timeframe)
        timeframe_str = timeframe_str.lower()
        
        # FIXED: Updated mapping for new frequency aliases
        timeframe_to_seconds_map = {
            "20s": 20, "1min": 60, "3min": 180, "5min": 300, "15min": 900,
        ...
        if timeframe_str in timeframe_to_seconds_map:
            return timeframe_to_seconds_map[timeframe_str]
        
        # Fallback for simple parsing if not in map
        try:
            if 's' in timeframe_str.lower():
                return int(timeframe_str.lower().replace('s', ''))
            if 'min' in timeframe_str.lower():
                return int(timeframe_str.lower().replace('min', '')) * 60
            if 'h' in timeframe_str.lower():
                return int(timeframe_str.lower().replace('h', '')) * 3600
            if 'd' in timeframe_str.lower():
                return int(timeframe_str.lower().replace('d', '')) * 86400
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n✅ REFINED configuration loading complete with weighted reward system for proper PPO training.")
        ...
        print("  - FIXED deprecated pandas frequency aliases (T->min, H->h, S->s)")
        print("  - FIXED timezone-aware datetime usage")
