...
class StrategyConfig(BaseModel):
    ...
    # Multi-timeframe lookback periods
    lookback_periods: Dict[str, int] = Field(
        default_factory=lambda: {
            'price_1m': 80, 'ohlc_1m': 50, 'price_3m': 80, 'ohlcv_3m': 50,
            'price_5m': 70, 'price_15m': 50, 'ohlc_15m': 50, 'price_1h': 70,
            'context': 5
        }
    )
...
class GlobalConfig(BaseModel):
    ...
    base_path: str = Field(default_factory=lambda: os.getenv('BASE_PATH', "/content/drive/MyDrive/crypto_data/alpha_proof"))
    
    # Asset configuration
    primary_asset: str = Field(default="BTCUSDT", description="Primary trading asset")
