...
class StrategyConfig(BaseModel):
    """✅ ENHANCED: Strategy configuration with reward horizon and leverage awareness."""
    ...
    # ✅ FIXED: Configurable leverage with validation
    ...
    def lookback_periods(self) -> Dict[FeatureKeys, int]:
        """
        Dynamically generate and validate lookback periods.
        ...
        # --- Validation logic is now inside the property ---
        for key, value in periods.items():
            if value <= 0:
                raise ValueError(f"Lookback period for {key.value} must be positive, got {value}")
            if value > 1000:
                warnings.warn(f"Large lookback period for {key.value}: {value}. Consider reducing for memory efficiency.")
    ...
    # ✅ FIXED: Risk management parameters with leverage awareness
    ...
    # ✅ FIXED: Leverage-aware maintenance margin rate
    ...
    def validate_leverage(cls, v):
        """✅ FIXED: Validate leverage and provide warnings for extreme values."""
        if v > 20:
    ...
    def validate_maintenance_margin(cls, v, values):
        """✅ FIXED: Ensure maintenance margin rate is appropriate for leverage."""
...
class PPOTrainingConfig(BaseModel):
    ...
    # ✅ FIXED: Enhanced parameters for tunable learning dynamics
...
class GlobalConfig(BaseModel):
    """✅ ENHANCED: Global configuration with reward horizon and leverage support."""
    ...
    def get_timeframe_seconds(self, timeframe: TimeFrameType | str) -> int:
        """
        ✅ FIXED: Converts a timeframe string (e.g., '1T', '4H') or enum to seconds.
    ...
    def get_required_warmup_period(self) -> int:
        """✅ FIXED: Calculate required warm-up period with leverage awareness."""
        try:
...
def create_config(env: Optional[Environment] = None, **overrides) -> GlobalConfig:
    """✅ ENHANCED: Create configuration with environment-specific settings, leverage, and reward horizon support."""
    try:
...
def validate_configuration(config: GlobalConfig) -> List[str]:
    """✅ ENHANCED: Validate configuration with leverage and reward horizon awareness."""
    warnings_list = []
    ...
    try:
        ...
        # ✅ FIXED: Enhanced strategy validation with leverage awareness
        ...
        # ✅ FIXED: Validate leverage-dependent parameters
...
def validate_and_fix_configuration(config: GlobalConfig) -> Tuple[GlobalConfig, List[str]]:
    """✅ FIXED: Validate configuration and automatically fix common issues with leverage awareness."""
    warnings_list = []
    ...
    try:
        ...
        # ✅ FIXED: Auto-adjust maintenance margin for leverage
        expected_margin_rate = max(0.005, 0.02 / config.strategy.leverage)
        if config.strategy.maintenance_margin_rate < expected_margin_rate * 0.5:
...
if __name__ == "__main__":
    print("--- Loading ENHANCED Configuration from .env and defaults ---")
    try:
        ...
        # ✅ FIXED: Display leverage configuration
        ...
        print("\n✅ ENHANCED Configuration loading and validation complete.")
