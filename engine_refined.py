...
class EnhancedHierarchicalTradingEnvironment(gym.Env):
    ...
    def __init__(self, df_base_ohlc: pd.DataFrame, normalizer: Normalizer, config=None,
        ...
        try:
            ...
            # Action and observation spaces remain the same
            ...
            # Initialize stateful features (including VWAP distance)
    ...
    def _update_stateful_features(self, step_index: int):
        ...
        for calc_cfg in self.strat_cfg.stateful_calculators:
            ...
            if latest_bar_timestamp > self.last_update_timestamps[timeframe]:
                ...
                # REFINEMENT: Special handling for VWAP distance calculator
                if calc_cfg.class_name == 'StatefulVWAPDistance':
                    # VWAP distance needs both price and volume
                    ...
                    # Standard update for other calculators
    ...
    def step(self, action: np.ndarray):
        ...
        try:
            ...
            # REFINEMENT: Store enhanced state for path-aware reward calculation
            pending_info = {
                ...
                # REFINEMENT: Initialize portfolio path tracking
                'portfolio_path': [initial_portfolio_value]
            }
            
            # REFINEMENT: Update existing pending rewards with current portfolio value
            ...
            # REFINEMENT: Path-Aware Delayed Reward Calculation
            reward = 0.0
            reward_components = {}
            horizon_info = {}
            
            # Calculate reward only if the buffer is full (we have N steps of history)
            if len(self.pending_rewards) == self.pending_rewards.maxlen:
                past_info = self.pending_rewards.popleft()  # Get the oldest action info
                
                # Add the final portfolio value to complete the path
                past_info['portfolio_path'].append(next_portfolio_value)
                
                # REFINEMENT: Choose reward calculation method based on configuration
                if self.use_path_aware_rewards and len(past_info['portfolio_path']) > 2:
                    # Use path-aware reward that considers the entire trajectory
                    ...
                    # Use traditional reward calculation
            ...
            # REFINEMENT: Enhanced terminal reward handling with path awareness
            if terminated or truncated:
                final_reward = reward
                
                while self.pending_rewards:
                    past_info = self.pending_rewards.popleft()
                    # Complete the path with terminal value
                    past_info['portfolio_path'].append(next_portfolio_value)
            ...
            info = {
                ...
                # REFINEMENT: Enhanced reward horizon info for debugging and analysis
                'reward_horizon': {
    ...
    # Additional methods remain unchanged but are included for completeness
    def reset(self, seed=None, options=None):
        ...
        try:
            ...
            # REFINEMENT: Clear the enhanced pending rewards buffer on reset
    ...
    # (Warmup, observation generation, feature extraction, etc.)
    ...
    def _get_single_step_observation(self, step_index) -> dict:
        ...
        try:
            ...
            # Context features (including VWAP distance)
    ...
    def get_performance_metrics(self) -> Dict[str, float]:
        ...
        try:
            ...
            # REFINEMENT: Additional path-aware reward metrics
            path_aware_metrics = {}
