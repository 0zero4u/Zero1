...
class RewardManager:
    ...
    def _calculate_raw_components(self, **kwargs) -> Dict[str, float]:
        ...
        # --- START OF ACTION CLARITY FIX ---
        # The original logic had a "dead zone" for actions < 0.01, which
        # taught the agent to always output a size of 0 to avoid penalties.
        # This new logic removes the "if" condition, creating a smooth,
        # continuous reward for increasing action size from zero. This encourages
        # exploration and helps the agent break out of the "do nothing" trap.
        components['action_clarity'] = (action_size ** 2) * 0.5
        # --- END OF ACTION CLARITY FIX ---

        thrashing_ratio = kwargs.get('thrashing_ratio', 0.0)
        if thrashing_ratio > 0.4:
    ...
    def _transform_to_ratios(self, raw_components: Dict[str, float], **kwargs) -> Dict[str, float]:
        ...
        # Penalties that are already ratios or unitless
        ...
        # Action clarity is already a unitless score
...
class FixedHierarchicalTradingEnvironment(gymnasium.Env):
    ...
    def __init__(self, df_base_ohlc: pd.DataFrame, normalizer: Normalizer, config=None,
        ...
        try:
            ...
            self.COLOR_GREEN, self.COLOR_RED, self.COLOR_YELLOW, self.COLOR_CYAN, self.COLOR_MAGENTA, self.COLOR_RESET, self.COLOR_BOLD = '\033[92m', '\033[91m', '\033[93m', '\033[96m', '\033[95m', '\033[0m', '\033[1m'
            ...
            self.action_space = spaces.Box(
                low=np.array([-1.0, -1.0], dtype=np.float32), # Symmetrical space for size
    ...
    def _print_attempted_trade_info(self, step: int, price: float, portfolio_value: float, unrealized_pnl: float, action: np.ndarray):
        """Prints a log when the agent wants to trade but the size is too small."""
        if not self.verbose: return
        pnl_color = self.COLOR_GREEN if unrealized_pnl >= 0 else self.COLOR_RED
        action_signal, action_size = action[0], action[1]
        
        log_message = (
            f"{self.COLOR_MAGENTA}🔎 ATTEMPT | Step {step:<7} | "
            f"Price: ${price:,.2f} | "
            f"Portfolio: ${portfolio_value:,.2f} | "
            f"PnL: {pnl_color}${unrealized_pnl:+.2f}{self.COLOR_RESET} | "
            f"Action (Sig/Size): {action_signal:+.2f}/{action_size:.2f} (Too Small)"
        )
        print(log_message)
    ...
    def step(self, action: np.ndarray):
        try:
            ...
            # --- START OF SYMMETRIC ACTION SPACE FIX ---
            # The network now outputs size in [-1, 1]. We remap it to [0, 1].
            # This prevents the "gravity well" at zero and allows for smooth exploration.
            action_signal = np.clip(action[0], -1.0, 1.0)
            action_size_raw = action[1] 
            action_size = (np.clip(action_size_raw, -1.0, 1.0) + 1.0) / 2.0
            # --- END OF SYMMETRIC ACTION SPACE FIX ---
            ...
            # --- START OF EXPLOIT HARDENING FIX ---
            # Define an "executed trade" based on a meaningful action size.
            # This prevents the agent from making microscopic trades to reset the
            # inactivity counter, effectively closing the "reward farming" loophole.
            is_executed_trade = action_size >= 0.01
            # --- END OF EXPLOIT HARDENING FIX ---
            ...
            reward, reward_info = self.reward_manager.calculate_final_reward(
                ...
                action=np.array([action_signal, action_size]), # Pass transformed action
            ...
            if self.verbose:
                log_action = np.array([action_signal, action_size])
                if is_executed_trade:
                    self._print_trade_info("TRADE", current_price, next_portfolio_value, next_unrealized_pnl, reward, raw_components_for_print)
                elif 0 < action_size < 0.01:
                    self._print_attempted_trade_info(self.current_step, next_price, next_portfolio_value, next_unrealized_pnl, log_action)
                elif (self.consecutive_inactive_steps == 1 or self.consecutive_inactive_steps % 50 == 0):
                    self._print_hold_info(self.current_step, next_price, next_portfolio_value, next_unrealized_pnl, log_action)
    ...
    def reset(self, seed=None, options=None):
        try:
            ...
            # --- START OF CRITICAL BUG FIX ---
            # These counters were not being reset, carrying over old values
            # to new episodes, corrupting the reward signal and causing
            # contradictory logs. This is the fix.
    ...
    def _log_live_state(self, action_signal, action_size, reward, reward_info, next_portfolio_value):
        """Writes the complete current state to a file for live monitoring."""
        if self.worker_id != 0:
            return
