...
import os
import torch
from datetime import datetime  # --- UPDATED: Added datetime import for versioning ---
...
class StrategyConfig(BaseModel):
    ...
    # REFINEMENT: Path-Aware Reward Horizon System
    ...
    # REFINEMENT: Precomputed features (reduced since dist_vwap_3m moved back to stateful)
    ...
    # REFINEMENT: Restored declarative calculators with VWAP distance
    stateful_calculators: List[StatefulCalculatorConfig] = Field(
        default_factory=lambda: [
            ...
            # REFINEMENT: Restored dist_vwap_3m to declarative pattern
            StatefulCalculatorConfig(
                name='vwap_dist_3m',
...
class GlobalConfig(BaseModel):
    ...
    # --- UPDATED: get_model_path method for versioning ---
...
def create_config(env: Optional[Environment] = None, **overrides) -> GlobalConfig:
    """
    Create configuration with environment-specific settings, leverage, reward horizon, and Transformer support.
    """
    try:
...
# Configuration validation and enhancement functions remain the same...
def validate_configuration(config: GlobalConfig) -> List[str]:
    """Validate configuration with leverage, reward horizon, and Transformer architecture awareness."""
    warnings_list = []
...
if __name__ == "__main__":
    print("--- Loading REFINED Configuration with Path-Aware Rewards ---")
    try:
        setup_environment()
        ...
        print(f"\n--- REFINED: Path-Aware Reward Horizon Configuration ---")
        ...
        print(f"\n--- REFINED: Declarative Feature Configuration ---")
        print("Context Features (restored declarative pattern):")
        ...
        print("\n✅ REFINED configuration loading complete with path-aware rewards and restored declarative pattern.")
