# FIXED: config.py - Immutable Config + Centralized Timeframe Parsing + Configurable Positional Encoding

"""
FIXED VERSION: Configuration with Critical Architecture Fixes

🚨 CRITICAL FIXES APPLIED:
1. Full configuration immutability (all config models now immutable)
2. Centralized timeframe parsing (no more duplicated logic)
3. Configurable positional encoding size for Transformer
4. Enhanced validation and error handling
5. Dependency injection pattern support

KEY IMPROVEMENTS:
- All Pydantic models are now immutable (allow_mutation = False)
- Single source of truth for timeframe string parsing
- Transformer architecture properly configurable
...
# --- FIXED: ENHANCED TRANSFORMER ARCHITECTURE CONFIG ---

class ModelArchitectureConfig(BaseModel):
    """
    FIXED: Configuration for the Transformer-based Neural Architecture with configurable parameters.
    
    KEY FIXES:
    - Added max_sequence_length for configurable positional encoding
    - Enhanced validation and error handling
    - Better parameter relationships and constraints
    """
    ...
    # FIXED: Configurable positional encoding size
    max_sequence_length: int = Field(default=2000, ge=100, le=10000, description="Maximum sequence length for positional encoding")
    ...
    def validate_max_sequence_length(cls, v):
        """Validate maximum sequence length is reasonable."""
        if v < 100:
            warnings.warn(f"Very small max_sequence_length ({v}), consider increasing for better performance")
        elif v > 5000:
            warnings.warn(f"Very large max_sequence_length ({v}), may impact memory usage")
        return v

    # FIXED: Configuration immutability
    class Config:
...
class StatefulCalculatorConfig(BaseModel):
    ...
    # Explicitly declare the output feature keys
...
class StrategyConfig(BaseModel):
    """Enhanced strategy configuration with Transformer architecture and reward horizon awareness."""
    
    # Path-Aware Reward Horizon System
    reward_horizon_steps: int = Field(
        default=1, ge=1, le=20,
        description="Number of steps for reward horizon. 1=immediate (20s), 9=3min horizon"
    )
    
    reward_horizon_decay: float = Field(
        default=0.95, ge=0.7, le=1.0,
        description="Decay factor for path-aware rewards. Lower values emphasize immediate returns more."
    )
    
    # Enable path-aware reward evaluation
    use_path_aware_rewards: bool = Field(
        default=True,
        description="Enable path-aware reward calculation that considers the entire trajectory"
    )
    ...
    # Stateful calculators with VWAP distance
    ...
    def get_reward_horizon_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'description': f"{horizon_minutes:.1f}-minute path-aware reward horizon" if self.reward_horizon_steps > 1 else "Immediate reward"
        }
...
class GlobalConfig(BaseModel):
    """
    FIXED: Enhanced global configuration with immutability and centralized timeframe parsing.
    
    KEY FIXES:
    - Full immutability with allow_mutation = False
    - Centralized get_pandas_freq() method
    - Enhanced validation and error handling
    """
    ...
    # --- FIXED: CENTRALIZED TIMEFRAME PARSING ---
    
    def get_pandas_freq(self, timeframe: TimeFrameType | str) -> str:
        """
        🚨 CRITICAL FIX: Centralized timeframe string parsing.
        
        This eliminates the duplicated logic across multiple files and provides
        a single source of truth for converting feature keys to pandas frequencies.
        
        Args:
            timeframe: TimeFrameType enum or string representation
        ...
        try:
            ...
            timeframe_str = timeframe.value if isinstance(timeframe, TimeFrameType) else str(timeframe)
            timeframe_str = timeframe_str.upper()
            
            # FIXED: Centralized mapping for all timeframe conversions
            timeframe_to_pandas_map = {
                "20S": "20S", 
                "1T": "1T", "1M": "1T",  # Support both notations
                "3T": "3T", "3M": "3T",
                "5T": "5T", "5M": "5T", 
                "15T": "15T", "15M": "15T",
                "1H": "1H", 
            ...
            # Handle feature key parsing (e.g., 'price_1m' -> '1T')
            if '_' in timeframe_str:
                # Extract timeframe from feature keys like 'price_1m', 'ohlc_15m'
                timeframe_part = timeframe_str.split('_')[-1]
                timeframe_str = timeframe_part.replace('s', 'S').replace('m', 'T').replace('h', 'H').upper()
            
            # Return mapped frequency
            if timeframe_str in timeframe_to_pandas_map:
                return timeframe_to_pandas_map[timeframe_str]
            else:
                logger.warning(f"Unknown timeframe '{timeframe_str}', defaulting to '20S'")
                return "20S"
    ...
    # --- END CENTRALIZED TIMEFRAME PARSING ---
    ...
    def get_required_warmup_period(self) -> int:
        ...
        try:
            ...
            for key, lookback in self.strategy.lookback_periods.items():
                ...
                # FIXED: Use centralized timeframe parsing
                pandas_freq = self.get_pandas_freq(key.value)
                seconds_per_bar = self.get_timeframe_seconds(pandas_freq)
...
def create_config(env: Optional[Environment] = None, **overrides) -> GlobalConfig:
    """
    FIXED: Create configuration with environment-specific settings and full immutability.
    ...
    Returns:
        Immutable GlobalConfig instance
    """
    try:
        ...
        base_config = {
            ...
            Environment.STAGING: {
                ...
                "log_level": "INFO",
                "strategy": {
                    "reward_horizon_steps": 3,
                    "reward_horizon_decay": 0.9,
                    "use_path_aware_rewards": True
                }
            },
            Environment.PRODUCTION: {
                ...
                "enable_wandb": True,
                "strategy": {
                    "reward_horizon_steps": 9,
                    "reward_horizon_decay": 0.95,
                    "use_path_aware_rewards": True
                }
            }
        }
        ...
        logger.info(f"✅ FIXED: Created immutable configuration for {base_env.value}")
        logger.info(f"Reward horizon: {reward_info['description']}")

        # Log architecture configuration
        arch = config.strategy.architecture
        logger.info(f"Transformer: d_model={arch.transformer_d_model}, max_seq_len={arch.max_sequence_length}")

        return config

    except Exception as e:
        logger.error(f"Error creating FIXED configuration: {e}")
        # Return basic config as fallback
        return GlobalConfig()

# REMOVED: Global mutable SETTINGS instance
# This eliminates the dangerous global mutable state that could cause runtime issues

# Configuration validation and enhancement functions
def validate_configuration(config: GlobalConfig) -> List[str]:
    """FIXED: Validate configuration with enhanced checks."""
    warnings_list = []
    ...
    try:
        ...
        # Reward horizon validation
        reward_horizon = config.strategy.reward_horizon_steps
        if reward_horizon > 15:
            warnings_list.append(f"Very long reward horizon ({reward_horizon} steps) may slow learning")

        if reward_horizon > 1 and config.strategy.reward_horizon_decay < 0.9:
            warnings_list.append(f"Low reward decay ({config.strategy.reward_horizon_decay}) may cause instability")

        # FIXED: Transformer architecture validation
        arch = config.strategy.architecture
        ...
        # FIXED: Positional encoding validation
        if arch.max_sequence_length < config.strategy.sequence_length * 2:
            warnings_list.append(f"Max sequence length ({arch.max_sequence_length}) may be too small for sequence length ({config.strategy.sequence_length})")
...
if __name__ == "__main__":
    print("--- FIXED Configuration with Critical Architecture Fixes ---")
    try:
        setup_environment()

        # FIXED: Create configuration with dependency injection pattern
        config = create_config()

        print(f"Environment: {config.environment.value}")
        print(f"Primary Asset: {config.primary_asset}")
        print(f"Base Path: {config.base_path}")

        # Display reward horizon configuration
        reward_info = config.strategy.get_reward_horizon_info()
        print(f"\n--- FIXED: Reward Horizon Configuration ---")
        ...
        print(f"Path-Aware Rewards: {reward_info['use_path_aware']}")

        # Display architecture configuration
        arch = config.strategy.architecture
        print(f"\n--- FIXED: Transformer Architecture Configuration ---")
        print(f"D_Model: {arch.transformer_d_model}")
        print(f"N_Heads: {arch.transformer_n_heads}")
        print(f"Num_Layers: {arch.transformer_num_layers}")
        print(f"FIXED: Max Sequence Length: {arch.max_sequence_length}")

        # FIXED: Test centralized timeframe parsing
        print(f"\n--- FIXED: Centralized Timeframe Parsing Test ---")
        test_timeframes = ['price_1m', '3T', TimeFrameType.HOUR_1, 'ohlc_15m', '20S']
        for tf in test_timeframes:
            pandas_freq = config.get_pandas_freq(tf)
            print(f"  {tf} -> {pandas_freq}")
        ...
        print("\n✅ FIXED configuration loading complete!")
        print("🚨 CRITICAL FIXES APPLIED:")
        print("  - Full configuration immutability (all models)")
        print("  - Centralized timeframe parsing (DRY principle)")
        print("  - Configurable Transformer positional encoding")
        print("  - Enhanced validation and error handling")
        print("  - Dependency injection support")

    except Exception as e:
        logger.error(f"FIXED Configuration test failed: {e}")
