...
class EnhancedFixedTrainer:
    ...
    def optimize(self, n_trials: int = 50, timeout: Optional[int] = None):
        ...
        try:
            ...
            # Use a persistent SQLite database for the study to allow resuming.
            study_name = f"zero1-ppo-optimization-{SETTINGS.primary_asset}"
            storage_path = SETTINGS.get_optuna_db_path()
            storage_name = f"sqlite:///{storage_path}"
            
            logger.info(f"💾 Optuna study '{study_name}' will be saved to: {storage_path}")

            study = optuna.create_study(
                study_name=study_name,
                storage=storage_name,
                load_if_exists=True,  # CRITICAL: This enables resuming the study
                direction='maximize',
                sampler=optuna.samplers.TPESampler(seed=42)
            )
            # --- END OF MODIFICATION ---
            
            # Log the number of trials already completed
            completed_trials = len([t for t in study.trials if t.state == optuna.trial.TrialState.COMPLETE])
            logger.info(f"📈 Found {completed_trials} completed trials in the study. Resuming optimization.")
            
            # The number of trials to run is the target minus what's already done
            remaining_trials = n_trials - completed_trials
            if remaining_trials <= 0:
                logger.info("🎯 Target number of trials already completed. Skipping optimization.")
            else:
                logger.info(f"🚀 Starting optimization for Enhanced Fixed Reward System ({remaining_trials} new trials)")
                study.optimize(self.objective, n_trials=remaining_trials, timeout=timeout, show_progress_bar=True)
