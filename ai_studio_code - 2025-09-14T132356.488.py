...
4.  **Penalty & Bonus Logic REFINED:**
    ...
    *   **Exploration Bonus:** Cost-based cap logic fixed to be a proportional reduction,
      not a hard cap that nullified the reward.
...
class RewardCalculator:
    ...
    def __init__(self, weights: Optional[dict] = None):
        ...
        self.weights = weights or {
            ...
            'exploration_bonus': 0.05, # Can be enabled for specific experiments
        }

    def calculate_exploration_bonus(self, trade_notional, prev_portfolio_value, estimated_costs) -> float:
        """FIXED: Proportional reduction instead of a hard cap."""
        # This is a placeholder for the more complex logic from the original file
        # The key fix is applied here.
        exploration_reward = (trade_notional / prev_portfolio_value) * 0.1 # Simplified base reward
        
        cost_ratio = estimated_costs / prev_portfolio_value
    ...
    def calculate_raw_components(self, **kwargs) -> Dict[str, float]:
        ...
        if current_drawdown > 0.08: # Smooth penalty threshold
    ...
    def step(self, action: np.ndarray):
        try:
            ...
            if len(self.action_frequency_buffer) == 100:
                ...
                vol_adjustment = np.clip(self.volatility_estimate * np.sqrt(252), 0.1, 0.4) # Annualize vol
            ...
            raw_components = self.reward_calculator.calculate_raw_components(
                base_return=(next_portfolio_value - initial_portfolio_value) / initial_portfolio_value,
