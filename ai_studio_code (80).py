...
class StrategyConfig(BaseModel):
    ...
    def lookback_periods(self) -> Dict[FeatureKeys, int]:
        ...
        The context lookback is derived from the length of context_feature_keys.
    ...
    # NEW: Declarative configuration for stateful feature calculators.
    # This replaces the hardcoded dictionary in engine.py, making config the single source of truth.
    stateful_calculators: List[StatefulCalculatorConfig] = Field(
        default_factory=lambda: [
            ...
            StatefulCalculatorConfig(
                ...
                params={'period': 100, 'num_levels': 1},  # Default support_resistance_window is 100
                timeframe='3T'
            ),
            StatefulCalculatorConfig(
                name='sr_15m',
    ...
    def get_required_warmup_period(self) -> int:
        ...
        try:
            ...
            for key, lookback in self.strategy.lookback_periods.items():
                if key in [FeatureKeys.CONTEXT, FeatureKeys.PORTFOLIO_STATE]:
                    continue
