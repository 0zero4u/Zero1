...
Major improvements:
- Fixed reward penalty issues with better scaling and progressive penalties
- Enhanced risk management with gradual liquidation and dynamic sizing
- Improved stability and robustness with better error handling
- Added regime detection and advanced market microstructure features
- Enhanced monitoring and performance attribution

Key fixes:
1. Reward function now uses proper normalization and bounded penalties
2. Progressive drawdown penalties instead of harsh step functions
3. Transaction cost consideration in rewards
4. Gradual margin call handling instead of immediate liquidation
5. Dynamic position sizing based on market volatility
6. Enhanced feature engineering with regime detection
7. Better risk management with portfolio heat limits
"""
...
class EnhancedRiskManager:
    ...
    def calculate_dynamic_position_limit(self, volatility: float, portfolio_value: float, 
        ...
        return base_limit * vol_adjustment * regime_adjustment
...
class AdvancedRewardCalculator:
    """Enhanced reward calculator with proper scaling and progressive penalties"""
    ...
    def calculate_enhanced_reward(self, prev_value: float, curr_value: float, 
        ...
        """
        Enhanced reward calculation with proper scaling and progressive penalties
        
        Returns:
            reward: Bounded reward value [-5, 5]
        ...
        try:
            ...
            # 1. Base return component (normalized)
            period_return = (curr_value - prev_value) / max(prev_value, 1e-6)
            self.return_buffer.append(period_return)
            
            # Normalize return using tanh to bound it
            normalized_return = np.tanh(period_return * 100)  # Scale then bound to [-1, 1]
            ...
            if len(self.return_buffer) >= 30:
                ...
                excess_returns = returns_array - (self.cfg.strategy.leverage * 0.02/252)  # Risk-free rate
...
class EnhancedHierarchicalTradingEnvironment(gym.Env):
    ...
    - Fixed reward penalty issues
    - Advanced risk management with gradual liquidation
    ...
    def __init__(self, df_base_ohlc: pd.DataFrame, normalizer: Normalizer, config=None):
        ...
        logger.info(f" -> Progressive Reward System: ON") 
        ...
        try:
            ...
            self.risk_manager = EnhancedRiskManager(self.cfg)
            self.reward_calculator = AdvancedRewardCalculator(self.cfg)
            ...
            logger.info("Enhanced environment initialized successfully.")
    ...
    def step(self, action: np.ndarray):
        """Enhanced step function with advanced risk management and reward calculation"""
        try:
            ...
            # Enhanced margin call handling with progressive liquidation
    ...
    def _get_single_step_observation(self, step_index) -> dict:
        ...
        try:
            ...
            # Enhanced portfolio state with market regime
            unrealized_pnl = self.asset_held * (current_price - self.entry_price)
...
if __name__ == "__main__":
    ...
    try:
        logger.info("Testing enhanced trading environment...")
        
        # Test the enhanced reward calculator
        reward_calc = AdvancedRewardCalculator(SETTINGS)
        ...
        # Test risk manager
        risk_mgr = EnhancedRiskManager(SETTINGS)
        regime = risk_mgr.update_market_regime(np.random.randn(50) * 0.01, 0.02)
        logger.info(f"Test market regime: {regime}")
