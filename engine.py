...
class HierarchicalTradingEnvironment(gym.Env):
    ...
    def __init__(self, df_base_ohlc: pd.DataFrame, config=None):
        super().__init__()
    ...
    def _precompute_market_features(self):
        ...
        try:
            ...
            # UPDATED: Define the final list of 13 context features
            feature_cols = [
            ...
            dummy_features = pd.DataFrame(
                np.zeros((len(self.base_timestamps), 13)),
        ...
        try:
            ...
            for key_enum, lookback in self.strat_cfg.lookback_periods.items():
                ...
                if key.startswith('price_'):
                    ...
                    # Normalize by recent average volume (as delta is a component of volume)
                    vol_window = df_tf.iloc[start_idx : end_idx + 1]['volume'].values.astype(np.float32)
                    avg_vol = np.mean(vol_window) + 1e-8
                    obs[key] = window / avg_vol
