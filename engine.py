...
class HierarchicalTradingEnvironment(gym.Env):
    ...
    - Centralized, pre-fitted normalization
    ...
    def __init__(self, df_base_ohlc: pd.DataFrame, normalizer: Normalizer, config=None):
        ...
        try:
            ...
            # Precompute market features (this part can proceed without normalizer)
            ...
            for key, lookback in self.strat_cfg.lookback_periods.items():
                ...
                elif key_str.startswith('price_') or key_str.startswith('volume_delta_'): # UPDATED
    ...
    def _precompute_market_features(self):
        ...
        try:
            ...
            # NEW: VWAP Distance Feature (3M timeframe)
            df_3m = self.timeframes['3T'].copy()
            if 'vwap' in df_3m.columns:
                df_3m['dist_vwap_3m'] = (df_3m['close'] - df_3m['vwap']) / (df_3m['vwap'] + 1e-9)
            else:
                logger.warning("VWAP column not found for 3T timeframe. VWAP distance feature will be zero.")
                df_3m['dist_vwap_3m'] = 0.0

            # Merge features with base timeframe
            base_df_with_features = self.timeframes[self.cfg.base_bar_timeframe.value].copy()
            ...
            base_df_with_features = pd.merge_asof(
                base_df_with_features, df_3m[['dist_vwap_3m']], left_index=True, right_index=True
            )
            ...
            for freq, plan in sr_plan.items():
                ...
                sr_df = pd.DataFrame(renamed_sr, index=df_tf.index)
            ...
            # UPDATED: Get the list of context features from the strategy config
            feature_cols = self.strat_cfg.context_feature_keys

            # Ensure all columns exist, fill with 0 if not
            for col in feature_cols:
                if col not in base_df_with_features.columns:
                    base_df_with_features[col] = 0.0
                    logger.warning(f"Context feature '{col}' was missing, filled with zeros.")
            ...
            num_context_features = len(self.strat_cfg.context_feature_keys)
            dummy_features = pd.DataFrame(
                np.zeros((len(self.base_timestamps), num_context_features)),
                index=self.base_timestamps,
                columns=self.strat_cfg.context_feature_keys
            )
    ...
    def _get_single_step_observation(self, step_index) -> dict:
        ...
        try:
            ...
            for key_enum, lookback in self.strat_cfg.lookback_periods.items():
                ...
                if key == 'context':
                    try:
                        raw_obs[key] = self.features_df.loc[current_timestamp].values.astype(np.float32)
                    except KeyError:
                        raw_obs[key] = np.zeros(lookback, dtype=np.float32)
                    continue
