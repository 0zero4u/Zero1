"""
OPTIMIZED: High-Performance Trading Environment with NumPy and Pre-computation

CRITICAL OPTIMIZATIONS APPLIED:
1. CONVERTED all Pandas DataFrames to NumPy arrays during initialization for near-instant data access.
2. PRE-CALCULATED an index mapping between the base timeframe and all resampled timeframes.
3. ELIMINATED on-the-fly stateful feature calculation by using pre-computed features passed at initialization.
4. REPLACED all Pandas .iloc and .get_indexer calls with direct, high-speed NumPy integer indexing.
5. REMOVED all "warmup" and incremental update logic, streamlining the `reset` and `step` methods.

KEY CHANGE: The environment is now significantly faster due to the removal of Pandas overhead in the training loop.
"""
...
class FixedRewardCalculator:
    ...
    def __init__(self, config, leverage: float = 10.0, reward_weights: Dict[str, float] = None):
        ...
        self.weights = reward_weights or {
            'base_return': 1.8,      # Primary signal
            'risk_adjusted': 0.11,   # Moderate Sharpe-like component
            'stability': 0.04,        # Reward consistency
            'transaction_penalty': -0.03,  # Conservative penalty
            'drawdown_penalty': -0.0006,      # Moderate drawdown deterrent
            'position_penalty': -0.01,     # Light position size penalty
            'risk_bonus': 0.04,       # Reward good risk management
            'exploration_bonus': 0.12, # Light exploration incentive
...
class FixedHierarchicalTradingEnvironment(gym.Env):
    """
    OPTIMIZED: High-performance Gymnasium environment using NumPy and pre-computation.
    
    CRITICAL OPTIMIZATIONS:
    1. All data is converted to NumPy arrays at initialization for high-speed access.
    2. On-the-fly feature calculation is eliminated; all features are pre-computed.
    3. All Pandas operations are removed from the `step` and `reset` methods.
    """
    ...
    def __init__(self, df_base_ohlc: pd.DataFrame, normalizer: Normalizer, config=None,
        ...
        logger.info("--- Initializing OPTIMIZED High-Performance Trading Environment ---")
        logger.info(f" -> OPTIMIZED: Using NumPy arrays for all time-series data")
        logger.info(f" -> OPTIMIZED: Using pre-computed features (no on-the-fly calculation)")
        ...
        try:
            ...
            # OPTIMIZATION: Process and store all pre-computed features as NumPy arrays
            if precomputed_features is not None:
                features_indexed = precomputed_features.set_index('timestamp')
                # Align with base timestamps to ensure perfect row-for-row correspondence
            ...
            logger.info("✅ OPTIMIZED environment initialized successfully.")
            
        except Exception as e:
            logger.error(f"Failed to initialize optimized environment: {e}", exc_info=True)
            raise
    
    def step(self, action: np.ndarray):
        """
        OPTIMIZED: Step function using NumPy for high-speed execution.
        """
        try:
            # OPTIMIZATION: On-the-fly stateful feature calculation is removed.
            self._update_market_regime_and_volatility(self.current_step)
            
            # OPTIMIZATION: Direct NumPy array access
            ...
            logger.error(f"Error in optimized environment step: {e}", exc_info=True)
            observation = self._get_observation_sequence()
            info = {'portfolio_value': self.balance, 'error': True, 'leverage': self.leverage}
            return observation, -1.0, True, False, info
    
    def reset(self, seed=None, options=None):
        """Reset environment. Warmup is no longer needed due to pre-computation."""
        try:
            ...
            # OPTIMIZATION: Warmup methods are removed.
            ...
            info = {
                ...
                'leverage': self.leverage, 'optimized_warmup': True,
                'immediate_reward_system': True
            }
            ...
            logger.error(f"Error resetting optimized environment: {e}", exc_info=True)
            raise
    
    def _get_current_context_features(self, step_index: int) -> np.ndarray:
        """OPTIMIZED: Get context features for a step by direct NumPy array indexing."""
    ...
    def _update_market_regime_and_volatility(self, step_index: int):
        """OPTIMIZED: Update market regime using fast NumPy slicing."""
        try:
            if step_index >= 50:
                # OPTIMIZATION: Use pre-converted NumPy array
    ...
    def _get_single_step_observation(self, step_index) -> dict:
        """OPTIMIZED: Generate single step observation using NumPy arrays and indexers."""
        try:
            ...
            for key_enum, lookback in self.strat_cfg.lookback_periods.items():
                ...
                # OPTIMIZATION: Use pre-calculated index and direct NumPy slicing
                end_idx = step_index
                ...
                if key.startswith('price_'):
                    ...
                    # Stack columns to form the OHLC(V) array
            ...
            # OPTIMIZATION: Get features from pre-computed NumPy arrays
            ...
            logger.error(f"Error getting optimized observation for step {step_index}: {e}", exc_info=True)
    ...
    def get_performance_metrics(self) -> Dict[str, float]:
        ...
        try:
            ...
            base_metrics = {
                ...
                'optimized_warmup': True, # Always true for this system
                'immediate_reward_system': True,
            }
