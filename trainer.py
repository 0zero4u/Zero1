...
class OptimizedTrainer:
    ...
    def __init__(self, base_config: Optional[Dict] = None, use_wandb: bool = False):
        ...
        try:
            ...
            # --- REPLACED SLOW FEATURE GENERATION ---
            # The original slow loop that stepped through a dummy environment has been
            # replaced with a single call to our new, fast, vectorized function.
            # This is the core of the performance fix.

            logger.info("Generating context features for normalizer fitting (vectorized)...")
            # This function now lives in processor.py and uses efficient pandas operations.
            self.features_df = generate_stateful_features_for_fitting(
                self.bars_df, SETTINGS.strategy
            )
            
            # The normalizer needs a features_df that is perfectly aligned with bars_df.
            # We ensure this alignment here.
