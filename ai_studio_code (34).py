...
class OptimizedTrainer:
    ...
    def objective(self, trial: optuna.Trial) -> float:
        ...
        try:
            ...
            if perf_callback.portfolio_values:
                ...
                # MODIFIED: Return tuple for multi-objective optimization.
                # We return Calmar Ratio, Total Return, and negative Max Drawdown (to minimize it).
                return calmar_ratio, total_return, -max_drawdown

            else:
                # MODIFIED: Return tuple for poor performance
                return -1.0, -1.0, -1.0
            ...
            # MODIFIED: Return tuple for failed trials, applying a heavy penalty to all objectives.
    ...
    def optimize(self, n_trials: int = 50, timeout: Optional[int] = None) -> optuna.Study:
        ...
        study = optuna.create_study(
            # MODIFIED: Specify multiple directions for objectives
            directions=['maximize', 'maximize', 'maximize'], # Corresponds to (calmar, return, -drawdown)
            pruner=pruner,
        ...
        # Store best results
        # MODIFIED: For multi-objective, we store all non-dominated trials (the Pareto front)
        self.best_trial_results = {
            'best_trials': study.best_trials,
            'n_trials': len(study.trials)
        }

        # Print results for all non-dominated trials on the Pareto front
        print("\n🎯 Optimization Results (Pareto Front):")
        print(f"Found {len(study.best_trials)} non-dominated solutions.")
        for i, trial in enumerate(study.best_trials):
            print(f"\n--- Solution {i+1} (Trial #{trial.number}) ---")
            print(f"  Values (Calmar, Return, -Drawdown): ({trial.values[0]:.3f}, {trial.values[1]:.3%}, {trial.values[2]:.3f})")
            print(f"  Params:")
    ...
    def train_best_model(self, study: optuna.Study, total_timesteps: int = 500000) -> PPO:
        ...
        # MODIFIED: Select the best trial from the Pareto front.
        # We will select the one with the highest Calmar Ratio (the first objective).
        if not study.best_trials:
            raise ValueError("Optimization study has no best trials to train from.")
        
        best_trial = max(study.best_trials, key=lambda t: t.values[0])
        print(f"Selected best trial #{best_trial.number} from Pareto front (highest Calmar Ratio: {best_trial.values[0]:.4f}).")
        ...
        try:
            ...
            if self.use_wandb:
                wandb_callback = WandbCallback(
                    ...
                    experiment_name=experiment_name,
                    config={'final_training': True, **best_trial.params}
                )
                callbacks.append(wandb_callback)
...
class MultiAgentEnsembleTrainer:
    ...
    def __init__(self, n_agents: int = 3, base_config: Optional[Dict] = None):
        ...
        self.base_config = base_config or SETTINGS.dict().copy()
        self.agents = []

    def train_ensemble(self, study: optuna.Study, total_timesteps: int = 200000) -> List[PPO]:
        """Train ensemble of agents using best params with different seeds."""
        print(f"🤖 Training ensemble of {self.n_agents} agents using best hyperparameters...")

        trainer = OptimizedTrainer() # To access helper methods

        # MockTrial class for re-creating model from params
        class MockTrial:
        ...
        best_trial = max(study.best_trials, key=lambda t: t.values[0])
        print(f"Using best trial #{best_trial.number} for ensemble training.")

        for i in range(self.n_agents):
            print(f"\n--- Training agent {i+1}/{self.n_agents} ---")
            vec_env = None
            try:
                # Create parallel environments with a unique seed for this agent
                vec_env = SubprocVecEnv([trainer._make_env(j, seed=42 + i) for j in range(trainer.num_cpu)])

                # Create mock trial with best parameters and a new seed
                best_params = best_trial.params.copy()
                best_params['seed'] = 42 + i
                mock_trial = MockTrial(best_params)

                # Create agent using the same logic as the main trainer
                agent = trainer.create_model(mock_trial, vec_env)

                # Train agent
                agent.learn(total_timesteps=total_timesteps // self.n_agents, progress_bar=True)
    ...
    if __name__ == '__main__': # Protect entry point for multiprocessing
        ...
        if optimization_trials > 0:
            ...
            model = PPO(
                ...
                policy_kwargs={
                    'features_extractor_class': EnhancedHierarchicalAttentionFeatureExtractor,
                    'lstm_hidden_size': 128 # Default value
                },
                verbose=1,
                device=SETTINGS.device
            )
        ...
        if use_ensemble:
            ...
            ensemble_trainer = MultiAgentEnsembleTrainer(n_agents=3)
            if 'study' in locals():
                ensemble_trainer.train_ensemble(study, total_timesteps=150000)
            else:
                print("⚠️ Skipping ensemble training: No optimization study available.")
