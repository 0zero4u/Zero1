...
class EnhancedHierarchicalAttentionFeatureExtractor(BaseFeaturesExtractor):
    ...
    def __init__(self, observation_space: spaces.Dict, arch_cfg=None):
        try:
            ...
            # Use attention-enhanced expert heads
            ...
            # UPDATED: Context head now expects 12 features instead of 13
            self.context_head = AttentionEnhancedExpertHead(
                12,  # Updated from 13 to 12 (removed funding rate)
        ...
        try:
            for cell_name, cell in self.cells.items():
                ...
                if obs_key not in observations:
                    logger.warning(f"Missing observation key: {obs_key}")
                    continue
...
if __name__ == "__main__":
    ...
    try:
        ...
        dummy_obs_space = spaces.Dict({
            ...
            'context': spaces.Box(low=-np.inf, high=np.inf, shape=(10, 12), dtype=np.float32)
        })
