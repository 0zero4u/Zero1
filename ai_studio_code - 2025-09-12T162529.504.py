...
class StrategyConfig(BaseModel):
    ...
    reward_scaling_factor: float = Field(
        default=150.0, ge=50.0, le=500.0,
        description="Scaling factor for immediate rewards to improve gradient signals"
    )
    
    # --- START OF HYPERPARAMETER CORRECTION ---
    # NEW: Inactivity penalty parameters to encourage patient trading
    inactivity_grace_period_steps: int = Field(
        default=15, ge=0,
        description="Number of inactive steps before inactivity penalty starts (grace period)"
    )
    
    penalty_ramp_up_steps: int = Field(
        default=30, ge=1,
        description="Number of steps over which the inactivity penalty ramps up to its full value"
    )
...
class PPOTrainingConfig(BaseModel):
    ...
    ent_coef: float = Field(default=0.5, ge=0.0, le=1.0, description="Entropy coefficient")
    # --- END OF HYPERPARAMETER CORRECTION ---
