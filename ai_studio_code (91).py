...
from processor import create_bars_from_trades, EnhancedDataProcessor
...
class OptimizedTrainer:
    ...
    def __init__(self, base_config: Optional[Dict] = None, use_wandb: bool = False):
        ...
        try:
            ...
            # --- Manually generate features_df for normalizer fitting ---
            # This is necessary because the environment now calculates features incrementally.
            logger.info("Generating context features for normalizer fitting...")
            from tqdm import tqdm
            dummy_env = HierarchicalTradingEnvironment(self.bars_df, normalizer=None, config=SETTINGS)
            warmup_period = SETTINGS.get_required_warmup_period()

            # 1. Warm up the stateful feature calculators
            dummy_env._warmup_features(warmup_period)

            # 2. Collect features for the rest of the dataset
            all_features = []
            for i in tqdm(range(warmup_period, dummy_env.max_step + 1), desc="Collecting context features"):
                dummy_env._update_stateful_features(i)
                current_features = dummy_env._get_current_context_features()
                all_features.append(current_features)

            # 3. Create the features DataFrame
            self.features_df = pd.DataFrame(all_features, columns=SETTINGS.strategy.context_feature_keys)
            del dummy_env, all_features  # Clean up memory
    ...
    def _make_env(self, rank: int, seed: int = 0, trial_params: Optional[Dict] = None) -> Callable:
        ...
        def _init():
            try:
                ...
                # Pass the fitted normalizer to the environment
    ...
    def objective(self, trial) -> float:
        ...
        try:
            ...
            if perf_callback.portfolio_values:
                ...
                if 'S' in base_bar_str:
                    base_bar_seconds = int(base_bar_str.replace('S', ''))
                elif 'T' in base_bar_str:
                    base_bar_seconds = int(base_bar_str.replace('T', '')) * 60
                else: # Fallback
                    timeframe_to_seconds = {"H": 3600, "D": 86400}
                    base_bar_seconds = timeframe_to_seconds.get(base_bar_str[-1], 20)
                ...
                if max_drawdown > 0.01:
                    ...
                    # Reward low-drawdown strategies, but prevent infinite Calmar
                    calmar_ratio = annualized_return * 10 if annualized_return >= 0 else annualized_return
