...
1. REMOVED all references to the obsolete "reward horizon" system.
2. UPDATED the Optuna search space to optimize the new `reward_scaling_factor`.
3. REMOVED the `RewardHorizonAnalysisCallback` which is no longer functional.
4. FIXED the objective function to focus on a clean risk-adjusted return metric.
5. ELIMINATED crashing calls to non-existent config attributes and methods.
"""
...
# FIXED: The engine now uses the immediate reward system
from engine import FixedHierarchicalTradingEnvironment as EnhancedHierarchicalTradingEnvironment
from normalizer import Normalizer

logger = logging.getLogger(__name__)

# FIXED: Removed the obsolete RewardHorizonAnalysisCallback
...
class PerformanceMonitoringCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        try:
            # Log every 100 steps for better granularity
...
class WandbCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        """Log metrics to W&B."""
        # Log every 100 steps for more frequent updates
...
class OptimizedTrainer:
    """
    FIXED: Trainer compatible with the Immediate Reward System.
    """
    ...
    def __init__(self, base_config: Optional[Dict] = None, use_wandb: bool = False):
        ...
        try:
            logger.info("🔄 Pre-loading training data for Immediate Reward System...")
            processor = EnhancedDataProcessor(config=SETTINGS)

            self.bars_df = processor.create_enhanced_bars_from_trades("in_sample")

            # FIXED: Removed call to non-existent reward horizon validation method.
            # The immediate reward system does not require this check.
            ...
            # The `bars_df` contains simple precomputed features. We merge it with the
            # stateful context features to create a single, complete feature set.
            ...
            # The normalizer correctly uses both dataframes to fit its parameters.
            ...
            # FIXED: Log the relevant reward parameter
            logger.info(f"✅ Immediate Reward System scaling factor: {SETTINGS.strategy.reward_scaling_factor}")
    ...
    def _make_env(self, rank: int, seed: int = 0, trial_params: Optional[Dict] = None) -> Callable:
        """Environment factory for the Immediate Reward System."""
        def _init():
            try:
                config_overrides = {}
                leverage = 10.0
                ...
                if trial_params:
                    ...
                    reward_weights = {
                        'base_return': trial_params.get('reward_weight_base_return', 2.0),
                        'risk_adjusted': trial_params.get('reward_weight_risk_adjusted', 0.15),
                        'stability': trial_params.get('reward_weight_stability', 0.1),
                        'transaction_penalty': trial_params.get('reward_weight_transaction_penalty', -0.05),
                        'drawdown_penalty': trial_params.get('reward_weight_drawdown_penalty', -0.3),
                        'position_penalty': trial_params.get('reward_weight_position_penalty', -0.02),
                        'risk_bonus': trial_params.get('reward_weight_risk_bonus', 0.2),
                        'inactivity_penalty': trial_params.get('reward_weight_inactivity_penalty', -0.1)
                    }
                    config_overrides = {
                        'strategy': {
                            'max_margin_allocation_pct': trial_params.get('max_margin_allocation_pct', 0.04),
                            # FIXED: Optimize the new reward scaling factor
    ...
    def create_model(self, trial_params: Dict, vec_env: VecEnv) -> PPO:
        ...
        try:
            ...
            logger.info(f"Using validated Transformer config: d_model={arch_config.transformer_d_model}, n_heads={arch_config.transformer_n_heads}")
            ...
            model = PPO(
                ...
                n_steps=trial_params.get('n_steps', 2048), batch_size=trial_params.get('batch_size', 64),
                n_epochs=trial_params.get('n_epochs', 10), gamma=trial_params.get('gamma', 0.99),
                gae_lambda=trial_params.get('gae_lambda', 0.95), clip_range=trial_params.get('clip_range', 0.2),
                ent_coef=trial_params.get('ent_coef', 0.01), max_grad_norm=trial_params.get('max_grad_norm', 0.5),
    ...
    def objective(self, trial) -> float:
        """FIXED: Optuna objective for the Immediate Reward System."""
        ...
        try:
            trial_params = {
                ...
                'reward_weight_risk_adjusted': trial.suggest_float('reward_weight_risk_adjusted', 0.0, 0.5),
                'reward_weight_drawdown_penalty': trial.suggest_float('reward_weight_drawdown_penalty', -0.8, 0.0),
                'reward_weight_inactivity_penalty': trial.suggest_float('reward_weight_inactivity_penalty', -0.2, -0.03),
            }
            ...
            eval_callback = EvalCallback(
                eval_vec_env, best_model_save_path=str(Path(log_path) / f"best_model_trial_{trial.number}"),
            ...
            callbacks = [eval_callback, perf_callback]

            if self.use_wandb:
                callbacks.append(WandbCallback("crypto_trading_transformer", experiment_name, trial_params))

            # Train for a fixed number of steps for each trial
            ...
            if perf_callback.portfolio_values:
                ...
                # FIXED: Simplified and robust objective function (Calmar-like)
                if max_drawdown > 0.01:
                    final_score = total_return / max_drawdown
                else:
                    final_score = total_return * 10 if total_return >= 0 else -10.0
    ...
    def optimize(self, n_trials: int = 50, timeout: Optional[int] = None):
        """Run optimization with the Immediate Reward System."""
        if not OPTUNA_AVAILABLE:
            logger.error("Optuna not available, cannot run hyperparameter optimization")
            return None
        try:
            study = optuna.create_study(direction='maximize', sampler=optuna.samplers.TPESampler(seed=42))
            logger.info(f"🚀 Starting optimization for Immediate Reward System")
    ...
    def train_best_model(self, best_params: Dict, final_training_steps: int) -> PPO:
        ...
        try:
            ...
            if self.use_wandb:
                callbacks.append(WandbCallback("crypto_trading_final", f"final_train_{datetime.now().strftime('%Y%m%d_%H%M%S')}", best_params))
...
def train_model_advanced(optimization_trials: int = 20,
    ...
    """
    FIXED: Advanced training with Immediate Reward System. Returns the path to the trained model.
    """
    try:
        logger.info("🎯 Starting FIXED Training with Immediate Reward System")
        logger.info("✅ FIXES APPLIED: Removed all obsolete reward horizon logic.")
        # FIXED: Removed attempt to set a non-existent config attribute.
        ...
        if optimization_trials > 0 and OPTUNA_AVAILABLE:
            ...
            best_params = {
                ...
                'reward_scaling_factor': 200.0, 'reward_weight_base_return': 2.0,
                'reward_weight_risk_adjusted': 0.15, 'reward_weight_drawdown_penalty': -0.3,
                'reward_weight_inactivity_penalty': -0.1,
            }
