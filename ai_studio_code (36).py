...
class HierarchicalTradingEnvironment(gym.Env):
    ...
    def __init__(self, df_base_ohlc: pd.DataFrame):
        ...
        try:
            ...
            self.episode_positive_rewards = 0
    ...
    def step(self, action: np.ndarray):
        ...
        try:
            ...
            if abs(trade_quantity) > 1e-6: # A trade is considered to have happened
            ...
            if reward > 0:
                self.episode_positive_rewards += 1
            ...
            # --- CALCULATE LIVE METRICS for info dict ---
            step_win_rate = self.episode_positive_rewards / self.episode_step_counter if self.episode_step_counter > 0 else 0.0

            sharpe_ratio_live = 0.0
            if len(self.episode_returns) > 20: # Use a slightly larger window for stability
                returns_arr = np.array(self.episode_returns)
                mean_ret = np.mean(returns_arr)
                std_ret = np.std(returns_arr)
                if std_ret > 1e-8:
                    # Annualize assuming 1-minute bars, 24/7 crypto market
                    # (365 days * 24 hours * 60 minutes)
                    annualization_factor = np.sqrt(525600)
                    sharpe_ratio_live = (mean_ret / std_ret) * annualization_factor
            ...
            info = {
                ...
                # Added live metrics
                'trade_count': self.episode_trade_count,
                'step_win_rate': step_win_rate,
                'sharpe_ratio_live': sharpe_ratio_live,
            }
...
def create_bars_from_trades(period: str) -> pd.DataFrame:
    ...
    try:
        # This dynamic import is here to avoid circular dependencies if this file
        # is imported by processor.py, though that shouldn't happen.
