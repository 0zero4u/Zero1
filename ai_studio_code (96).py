...
REWARD_REFACTOR: Added reward function weights to hyperparameter search to automatically
balance PnL, drawdown, and churn penalties.
"""
    ...
    def _on_training_start(self) -> None:
        try:
            if WANDB_AVAILABLE and not self.run:
                self.run = wandb.init(project=self.project_name, name=self.experiment_name, config=self.config, sync_tensorboard=True)
        ...
        try:
            if self.run and self.locals.get('infos'):
                ...
                if 'episode' in self.locals:
                    ...
                    if episode_info: self.run.log({'episode/reward': episode_info.get('r', 0), 'episode/length': episode_info.get('l', 0), 'episode/time': episode_info.get('t', 0)})
                if 'portfolio_value' in info:
                    self.run.log({'portfolio/value': info['portfolio_value'], 'portfolio/balance': info.get('balance', 0), 'portfolio/asset_held': info.get('asset_held', 0), 'portfolio/drawdown': info.get('drawdown', 0), 'portfolio/volatility': info.get('volatility', 0)})
            return True
        except Exception as e:
...
class LiveRLMonitoringCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if current_time - self.last_log_time < self.log_interval_seconds: return True
        ...
        try:
            if self.n_calls % self.log_frequency == 0:
                if hasattr(self.model.policy.features_extractor, 'get_attention_analysis'):
                    ...
                    if analysis:
                        self.attention_history.append({'step': self.n_calls, 'analysis': analysis})
                        if 'expert_weights' in analysis:
                            for expert_name, weights in analysis['expert_weights'].items(): self.logger.record(f'attention/{expert_name}_weight', np.mean(weights))
                        if 'attention_entropy' in analysis: self.logger.record('attention/entropy', np.mean(analysis['attention_entropy']))
            return True
        except Exception as e:
...
class PerformanceMonitoringCallback(BaseCallback):
    def __init__(self, performance_threshold: float = -0.1, drawdown_threshold: float = 0.2, alert_callback: Optional[Callable] = None, verbose: int = 0):
        super().__init__(verbose)
            ...
            if self.locals.get('infos'):
                ...
                if 'portfolio_value' in info:
                    ...
                    if current_drawdown > self.drawdown_threshold:
                        if self.alert_callback: self.alert_callback(f"High drawdown detected: {current_drawdown:.2%}")
                        if self.verbose > 0: logger.warning(f"Warning: Drawdown of {current_drawdown:.2%} exceeds threshold")
            return True
        except Exception as e:
...
class OptimizedTrainer:
    ...
    # --- UPDATED: _make_env now passes reward_weights ---
    def _make_env(self, rank: int, seed: int = 0, trial_params: Optional[Dict] = None) -> Callable:
        ...
        def _init():
            try:
                ...
                if trial_params:
                    if 'max_margin_allocation_pct' in trial_params:
                        config_overrides['strategy'] = {'max_margin_allocation_pct': trial_params['max_margin_allocation_pct']}
                    
                    # Extract reward weights from trial parameters
                    reward_weights_dict = {
                        'pnl_weight': trial_params.get('reward_pnl_weight', 100.0),
                        'drawdown_penalty_weight': trial_params.get('reward_drawdown_penalty_weight', 1.5),
                        'churn_penalty_weight': trial_params.get('reward_churn_penalty_weight', 0.1)
                    }

                trial_specific_config = create_config(**config_overrides)
                
                # Pass the fitted normalizer AND the reward weights to the environment
    ...
    def create_model(self, trial_params: Dict, vec_env: VecEnv) -> PPO:
        ...
        try:
            ...
            if batch_size > n_steps: batch_size = n_steps // 2
            arch_config = {'lstm_layers': lstm_layers, 'expert_lstm_hidden_size': expert_hidden_size, 'attention_head_features': attention_features, 'dropout_rate': dropout_rate}
            policy_kwargs = {'features_extractor_class': EnhancedHierarchicalAttentionFeatureExtractor, 'features_extractor_kwargs': {'arch_cfg': type('Config', (), arch_config)()}, 'lstm_hidden_size': lstm_hidden_size, 'net_arch': {'pi': [expert_hidden_size * 2, expert_hidden_size], 'vf': [expert_hidden_size * 2, expert_hidden_size]}}
            model = PPO("MlpLstmPolicy", vec_env, policy_kwargs=policy_kwargs, learning_rate=learning_rate, n_steps=n_steps, batch_size=batch_size, n_epochs=n_epochs, gamma=gamma, gae_lambda=gae_lambda, clip_range=clip_range, ent_coef=ent_coef, max_grad_norm=max_grad_norm, verbose=0, device=SETTINGS.device, seed=trial_params.get('seed', 42))
            return model
        except Exception as e:
            logger.error(f"Error creating model: {e}")
            raise

    # --- UPDATED: objective function now searches for reward weights ---
    def objective(self, trial) -> float:
        ...
        try:
            ...
            trial_params = {
                ...
                # --- NEW: Add reward weights to the search space ---
                'reward_pnl_weight': trial.suggest_float('reward_pnl_weight', 50.0, 200.0),
                'reward_drawdown_penalty_weight': trial.suggest_float('reward_drawdown_penalty_weight', 0.5, 5.0),
                'reward_churn_penalty_weight': trial.suggest_float('reward_churn_penalty_weight', 0.05, 0.5),
            }

            # Create parallel environments, now passing trial_params to configure reward weights
            ...
            # Setup logging and callbacks (logic is unchanged)
            ...
            eval_callback = EvalCallback(eval_vec_env, best_model_save_path=str(Path(log_path) / f"best_model_trial_{trial.number}"), log_path=log_path, eval_freq=max(5000 // self.num_cpu, 500), deterministic=True, render=False, n_eval_episodes=5)
            callbacks.append(eval_callback)
            perf_callback = PerformanceMonitoringCallback(performance_threshold=-0.15, drawdown_threshold=0.25)
            ...
            if self.use_wandb:
                wandb_callback = WandbCallback(project_name="crypto_trading_optimization", experiment_name=experiment_name, config={'trial_number': trial.number, **trial_params})
                callbacks.append(wandb_callback)
            training_steps = trial.suggest_int('total_timesteps', 50000, 200000, step=10000)
            class PruningCallback(BaseCallback):
                def __init__(self, trial, verbose: int = 0): super().__init__(verbose); self.trial = trial; self.evaluation_results = []
                def _on_step(self) -> bool:
                    try:
                        if self.n_calls % (10000 // self.training_env.num_envs) == 0 and len(perf_callback.portfolio_values) > 0:
                            ...
                            if len(recent_values) > 1:
                                ...
                                if self.trial.should_prune(): raise optuna.TrialPruned()
                        return True
                    except optuna.TrialPruned: raise
                    except Exception as e: logger.error(f"Error in pruning callback: {e}"); return True
            if OPTUNA_AVAILABLE: callbacks.append(PruningCallback(trial))
            
            # Train the model
            model.learn(total_timesteps=training_steps, callback=callbacks, progress_bar=False)

            # Calculate final performance metric (Calmar Ratio, unchanged and still a great choice)
            if perf_callback.portfolio_values:
                ...
                base_bar_str = SETTINGS.base_bar_timeframe.value
                if 'S' in base_bar_str: base_bar_seconds = int(base_bar_str.replace('S', ''))
                elif 'T' in base_bar_str: base_bar_seconds = int(base_bar_str.replace('T', '')) * 60
                else: timeframe_to_seconds = {"H": 3600, "D": 86400}; base_bar_seconds = timeframe_to_seconds.get(base_bar_str[-1], 20)
                ...
                if max_drawdown > 0.01: calmar_ratio = annualized_return / max_drawdown
                else: calmar_ratio = annualized_return * 10 if annualized_return >= 0 else annualized_return
            ...
            except Exception as e: logger.error(f"Error cleaning up environments: {e}")

    def optimize(self, n_trials: int = 50, timeout: Optional[int] = None):
        # (This method remains unchanged)
        if not OPTUNA_AVAILABLE: logger.error("Optuna not available..."); return None
        try:
            pruner = optuna.pruners.MedianPruner(n_startup_trials=5, n_warmup_steps=10, interval_steps=10)
            ...
            self.best_trial_results = {'best_trial': study.best_trial, 'best_value': study.best_value, 'n_trials': len(study.trials)}
            logger.info("🎯 Optimization Results:"); logger.info(f"Best value: {study.best_value:.4f}"); logger.info(f"Best params: {study.best_trial.params}")
            return study
        except Exception as e:
    ...
    def train_best_model(self, study_or_params, total_timesteps: int = 500000) -> PPO:
        # (This method remains unchanged, it automatically uses the best params including reward weights)
        ...
        try:
            ...
            model.set_logger(configure(log_path, ["stdout", "csv", "tensorboard"]))
            callbacks = []
            checkpoint_callback = CheckpointCallback(save_freq=max(10000 // self.num_cpu, 500), save_path=str(Path(log_path) / "checkpoints"), name_prefix="ppo_crypto_model")
            ...
            callbacks.append(attention_callback)
            if self.use_wandb:
                wandb_callback = WandbCallback(project_name="crypto_trading_final", experiment_name=experiment_name, config={'final_training': True, **best_params})
                callbacks.append(wandb_callback)
...
def train_model_advanced(optimization_trials: int = 20,
    ...
    try:
        ...
        print("="*50)
        if __name__ != '__main__': logger.warning("Training should be run as main script for multiprocessing safety")
        ...
        if optimization_trials > 0 and OPTUNA_AVAILABLE:
            ...
            if study is None:
                ...
                # --- UPDATED: Default params now include new reward weights ---
                best_params = {
                    'learning_rate': 3e-4, 'n_steps': 2048, 'batch_size': 64, 'n_epochs': 10, 'gamma': 0.99, 'gae_lambda': 0.95,
                    'clip_range': 0.2, 'ent_coef': 0.01, 'max_grad_norm': 0.5, 'lstm_layers': 2, 'expert_hidden_size': 32,
                    'attention_features': 64, 'dropout_rate': 0.1, 'lstm_hidden_size': 64, 'seed': 42, 'max_margin_allocation_pct': 0.02,
                    'reward_pnl_weight': 100.0, 'reward_drawdown_penalty_weight': 1.5, 'reward_churn_penalty_weight': 0.1
                }
            else:
...
if __name__ == "__main__":
    ...
    try:
        model = train_model_advanced(optimization_trials=5, final_training_steps=100000, use_wandb=False, use_ensemble=False)
