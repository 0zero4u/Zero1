...
class OptimizedTrainer:
    ...
    def __init__(self, base_config: Optional[Dict] = None, use_wandb: bool = False):
        ...
        try:
            ...
            # We need bars and features for fitting the normalizer
            self.bars_df = processor.create_enhanced_bars_from_trades("in_sample")
            
            # Create a dummy env just to pre-compute features
            # Pass None for normalizer as it's not needed for feature computation
            dummy_env = HierarchicalTradingEnvironment(self.bars_df, normalizer=None, config=SETTINGS)
            self.features_df = dummy_env.features_df
            del dummy_env
