...
class HierarchicalTradingEnvironment(gym.Env):
    ...
    better normalization, dynamic warm-up period calculation, and isolated margin simulation.
    """
    ...
    def __init__(self, df_base_ohlc: pd.DataFrame):
        ...
        print(f" -> Leverage: {self.strat_cfg.leverage}x | Maintenance Margin: {self.strat_cfg.maintenance_margin_rate:.2%}")
    ...
    def reset(self, seed=None, options=None):
        ...
        # MODIFIED: balance now represents total account equity
        self.balance = 10000.0
        self.asset_held = 0.0
        # NEW: State variables for isolated margin trading
    ...
    def step(self, action: np.ndarray):
        ...
        # --- NEW: ISOLATED MARGIN & LIQUIDATION LOGIC ---
        ...
        # --- REVISED TRADING LOGIC WITH LEVERAGE ---
        ...
        # Realize PnL from previous position and apply fees
        ...
        # Margin check (ensure used margin doesn't exceed equity)
        if self.used_margin > self.balance:
            # Not enough equity for this position, so close it immediately
            self.balance -= self.used_margin # This will result in negative balance
            self.asset_held = 0.0
            self.used_margin = 0.0
