    ...
    def objective(self, trial) -> float:
        ...
        try:
            ...
            trial_params = {
                ...
                # Reward weights for optimization
    ...
    try:
        ...
        if optimization_trials > 0 and OPTUNA_AVAILABLE:
            ...
            best_params = {
                ...
                # --- START OF AGGRESSIVE HYPER-THRASHING FIX ---
                # The goal is to break the "trade every step" loop by swapping the
                # blunt `frequency` penalty for the more precise `thrashing` penalty.
                'reward_weight_realized_pnl': 3.0,
                'reward_weight_unrealized_pnl_shaping': 0.1,
                'reward_weight_trade_cost': 0.75,      # Keep cost penalty to punish micro-trades.
                'reward_weight_drawdown': 1.0,         # Re-enable a moderate drawdown penalty.
                'reward_weight_thrashing': 3.0,        # STRONG PENALTY for changing its mind. This is the key.
                'reward_weight_frequency': 0.0,        # CRITICAL: Turn OFF the penalty that causes the loop.
                'reward_weight_inactivity': 1.5,       # Keep strong pressure to act, forcing it to find a new solution.
                'reward_weight_action_clarity': 25.0,  # Strongly reward breaking the "tiny size" habit.
                # --- END OF AGGRESSIVE HYPER-THRASHING FIX ---
            }
            # ---  ---
...
if __name__ == "__main__":
    ...
    train_model_fixed(
        optimization_trials=0, # Set to 0 to use the fixed `best_params`
