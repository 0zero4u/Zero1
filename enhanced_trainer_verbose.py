...
class RolloutInfoCallback(BaseCallback):
    def __init__(self, log_freq: int, verbose: int = 0):
        ...
        self.keys_to_log = ["drawdown", "raw_reward", "intrinsic_reward", "portfolio_value"]
    def _on_step(self) -> bool:
        for info in self.locals.get("infos", []):
            ...
            if weighted_rewards:
                for k, v in weighted_rewards.items(): self.buffers[f'reward_weighted/{k}'].append(v)
...
class EnhancedFixedTrainer:
    ...
    def create_model(self, trial_params: Dict, vec_env: VecEnv) -> SAC:
        ...
        return SAC(policy="MultiInputPolicy", env=vec_env,
    ...
    def optimize(self, n_trials: int = 50, timeout: Optional[int] = None):
        ...
        study = optuna.create_study(study_name=f"sac-optimization-{SETTINGS.environment.value}", storage=f"sqlite:///{db_path}",
    ...
    try:
        ...
        if optimization_trials > 0 and OPTUNA_AVAILABLE:
            ...
            best_params = {
                ...
                'batch_size': 256, 'tau': 0.005, 'gamma': 0.99, 'train_freq': (1, 'step'),
                'gradient_steps': 1, 'ent_coef': 'auto', 'transformer_d_model': 64, 'transformer_n_heads': 4,
                'transformer_num_layers': 2, 'dropout_rate': 0.1, 'seed': 42, 'leverage': 10.0,
                'reward_weight_realized_pnl': 3.0, 'reward_weight_unrealized_pnl_shaping': 0.1,
                'reward_weight_trade_cost': 0.35, 'reward_weight_drawdown': 0.0,
                'reward_weight_thrashing': 0.0, 'reward_weight_frequency': 0.1,
                'reward_weight_inactivity': 1.0, 'reward_weight_action_clarity': 0.15,
            }
