...
# --- START OF NEW/MODIFIED CODE: Replaced ComprehensiveLoggingCallback with two superior callbacks ---

class RolloutInfoCallback(BaseCallback):
    """
    Aggregates data from every step across all parallel environments
    and logs the aggregated statistics (mean) at a fixed interval,
    providing a smooth, continuous view of the agent's behavior.
    """
    def __init__(self, log_freq: int, verbose: int = 0):
        ...
        self.keys_to_log = [
            "action_magnitude", "thrashing_ratio", "drawdown",
            "consecutive_inactive_steps", "raw_reward", "intrinsic_reward",
    ...
    def _on_step(self) -> bool:
        for info in self.locals.get("infos", []):
            for key in self.keys_to_log:
                if key in info:
                    self.buffers[key].append(info[key])
            
            # Also log weighted reward components
            weighted_rewards = info.get('weighted_rewards', {})
            if weighted_rewards:
                for k, v in weighted_rewards.items():
                    self.buffers[f'reward_weighted/{k}'].append(v)

        if self.n_calls % self.log_freq == 0:
            for key, values in self.buffers.items():
                if values:
                    mean_val = np.mean(values)
                    self.logger.record(f'rollout/{key}_mean', mean_val)
...
class EpisodeStatsCallback(BaseCallback):
    """
    Correctly checks all parallel environments for episode completion
    and logs the final, true performance metrics for each episode as it ends.
    """
...
class EnhancedFixedTrainer:
    ...
    def objective(self, trial) -> float:
        ...
        try:
            ...
            eval_freq_base = max(learning_starts_val, 10_000)
            eval_freq = eval_freq_base // self.num_cpu
            logger.info(f"Trial {trial.number}: `learning_starts` is {learning_starts_val}. Setting `eval_freq` to {eval_freq} (base: {eval_freq_base}).")
            ...
            # --- START OF NEW/MODIFIED CODE: Use the new, superior callbacks ---
            callbacks = [
                eval_callback,
                HParamCallback(trial, ['eval/mean_portfolio_value']),
                RolloutInfoCallback(log_freq=eval_freq),
                EpisodeStatsCallback(verbose=0)
            ]
            # --- END OF NEW/MODIFIED CODE ---
    ...
    def train_best_model(self, best_params: Dict, final_training_steps: int) -> SAC:
        ...
        # --- START OF NEW/MODIFIED CODE: Use the new callbacks for final training ---
        callbacks = [
            RolloutInfoCallback(log_freq=max(5000 // self.num_cpu, 500)),
