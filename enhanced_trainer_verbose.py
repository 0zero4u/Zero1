...
class EnhancedFixedTrainer:
    ...
    def objective(self, trial) -> float:
        ...
        try:
            # --- START OF HYPERPARAMETER CORRECTION ---
            trial_params = {
                ...
                'ent_coef': trial.suggest_float('ent_coef', 0.2, 0.8, log=True),
                ...
                # Rebalanced Reward Weights: Prioritize profit, penalize hyperactivity
            ...
            # --- END OF HYPERPARAMETER CORRECTION ---
    ...
    def optimize(self, n_trials: int = 50, timeout: Optional[int] = None):
        ...
        try:
            # --- START OF FIX: Remove fixed seed from Optuna sampler ---
            # Using a fixed seed (seed=42) makes the entire hyperparameter search
            # deterministic. This prevents true exploration on subsequent runs.
            # Removing the seed allows Optuna to perform a different random search each time.
...
def train_model_fixed(optimization_trials: int = 20,
    ...
    try:
        ...
        if optimization_trials > 0 and OPTUNA_AVAILABLE:
            ...
            # --- START OF HYPERPARAMETER CORRECTION ---
            # These are pre-tuned, high-quality default parameters updated based on performance analysis.
            best_params = {
                ...
                # Transformer Architecture: Stable configuration
                ...
                # Strategy & Risk: Reduced risk profile
                'leverage': 10.0,
                'max_margin_allocation_pct': 0.04,
                
                # Reward Function: Balanced for profitability
                'reward_scaling_factor': 150.0,
                ...
                'reward_weight_frequency_penalty': -1.2,
