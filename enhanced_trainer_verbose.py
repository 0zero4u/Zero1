    ...
    def objective(self, trial) -> float:
        ...
        try:
            trial_params = {
                ...
                # --- START OF FIX: Added learning_starts to Optuna search space ---
            ...
            # The evaluation frequency MUST be >= learning_starts for SAC. Otherwise, Optuna
            # evaluates a random agent, providing a useless optimization signal.
            learning_starts_val = trial_params.get('learning_starts', 10_000)
            # We set a base evaluation interval (e.g., 10k steps) and ensure the first
            # evaluation happens at or after the agent has started learning.
            ...
            eval_callback = PortfolioValueEvalCallback(
                ...
                log_path=log_path, eval_freq=eval_freq, # Use the new, intelligent frequency
            ...
            callbacks = [
                ...
                ComprehensiveLoggingCallback(log_freq=eval_freq) # Align logging with evaluation
            ]
            if self.use_wandb:
