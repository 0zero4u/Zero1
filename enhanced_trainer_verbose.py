...
# --- START OF FIX ---
# Import the TimeLimit wrapper to ensure evaluation episodes finish.
...
class EnhancedFixedTrainer:
    ...
    def _make_env(self, rank: int, seed: int = 0, trial_params: Optional[Dict] = None) -> Callable:
        def _init():
            ...
            if trial_params:
                ...
                # FIXED: Use positive weights for penalties, as they are now negative values.
                reward_weights = {
    ...
    def objective(self, trial) -> float:
        ...
        try:
            # FIXED: Updated trial params to use positive weights for penalties.
            trial_params = {
                ...
                # The environment now makes penalty components negative, so weights must be positive.
            ...
            # --- START OF FIX ---
            # Create the raw environment first
            eval_env_raw = self._make_env(rank=0, seed=123, trial_params=trial_params)()
            # Wrap it with a TimeLimit to ensure episodes finish during evaluation
            eval_env_limited = TimeLimit(eval_env_raw, max_episode_steps=5000)
            # Create the vectorized environment from the time-limited one
...
def train_model_fixed(optimization_trials: int = 20,
    ...
    try:
        ...
        if optimization_trials > 0 and OPTUNA_AVAILABLE:
            ...
            # FIXED: Updated default params with positive weights for penalties.
            best_params = {
                ...
                # New balanced positive weights that multiply the negative penalty values.
