...
class PortfolioValueEvalCallback(EvalCallback):
    ...
    def _on_step(self) -> bool:
        # This is the standard EvalCallback logic
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            super()._on_step()

            # After the parent class runs evaluation, we can access the results.
            # The 'ep_info_buffer' will contain the 'info' dicts from the last step of each episode.
            if len(self.last_ep_info_buffer) > 0:
                # Extract the final portfolio value from the info dict of each episode
                portfolio_values = [info.get('portfolio_value', 0.0) for info in self.last_ep_info_buffer]
            ...
            # Dump the logs, including our custom one
    ...
    try:
        ...
        if optimization_trials > 0 and OPTUNA_AVAILABLE:
            ...
            best_params = {
                ...
                'reward_weight_transaction_penalty': -0.10,
                'reward_weight_drawdown_penalty': -1.2,
                'reward_weight_position_penalty': -0.05,
                'reward_weight_exploration_bonus': 0.15,
                'reward_weight_inactivity_penalty': -0.40,
                'reward_weight_frequency_penalty': -0.7,
