...
# --- START OF CRITICAL FIX: Custom EvalCallback ---
class PortfolioValueEvalCallback(EvalCallback):
    """
    A custom EvalCallback that evaluates the agent based on the final portfolio value
    (raw P&L) instead of the cumulative reward, which can be gamed.
    
    CORRECTED: This version implements its own evaluation loop to correctly capture
    the 'portfolio_value' from the info dictionary, fixing the AttributeError related
    to 'last_ep_info_buffer' which is not populated by the parent class's evaluation.
    """
    ...
    def _on_step(self) -> bool:
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            # --- START: Manual evaluation loop to replace super()._on_step() ---
            portfolio_values = []
            episode_rewards = []
            episode_lengths = []
            ...
            for _ in range(self.n_eval_episodes):
                ...
                # After episode is done, infos[0] contains the final info dict
            ...
            # --- END: Manual evaluation loop ---

            if self.verbose > 0:
                logger.info(f"Eval at timestep {self.num_timesteps}: "
                      f"mean_reward={mean_reward:.2f} +/- {std_reward:.2f}, "
                      f"mean_portfolio_value=${mean_portfolio_value:,.2f}")
            ...
            if mean_portfolio_value > self.best_mean_portfolio_value:
                ...
                if self.best_model_save_path:
                    ...
                    logger.info(f"Saved new best model to {self.best_model_save_path}")
    ...
    def _on_step(self) -> bool:
        ...
        try:
            if self.n_calls % 700 == 0 and self.locals.get('infos'):
                ...
                insignificant_trades = info.get('total_insignificant_trades', 0)
                self.logger.record('behavior/total_insignificant_trades', insignificant_trades)
                
                attempted_trades = info.get('total_attempted_trades', 0)
                executed_trades = info.get('total_executed_trades', 0)
                self.logger.record('behavior/total_attempted_trades', attempted_trades)
                self.logger.record('behavior/total_executed_trades', executed_trades)
                
                thrashing_ratio = attempted_trades / (executed_trades + 1e-8)
                self.logger.record('behavior/thrashing_ratio', thrashing_ratio)
...
class EnhancedFixedTrainer:
    ...
    def _make_env(self, rank: int, seed: int = 0, trial_params: Optional[Dict] = None) -> Callable:
        """
        Environment factory with DIAGNOSTIC FIX: Sets up dedicated logging for each worker.
        """
        def _init():
            worker_log_path = os.path.join(SETTINGS.get_logs_path(), f"worker_{rank}.log")
            worker_logger = logging.getLogger(f"worker_{rank}")
            worker_logger.setLevel(logging.DEBUG)
            worker_logger.propagate = False
            
            if not worker_logger.handlers:
                fh = logging.FileHandler(worker_log_path, mode='w')
                fh.setFormatter(logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s'))
                worker_logger.addHandler(fh)
            
            try:
                worker_logger.info(f"Worker {rank} initializing environment...")
                config_overrides = {}
                leverage = 10.0
                reward_weights = None
                ...
                if trial_params:
                    ...
                    reward_weights = {
                        'base_return': trial_params.get('reward_weight_base_return', 4.5),
                        'transaction_penalty': trial_params.get('reward_weight_transaction_penalty', -0.10),
                        'drawdown_penalty': trial_params.get('reward_weight_drawdown_penalty', -1.2),
                        'position_penalty': trial_params.get('reward_weight_position_penalty', -0.05),
                        'exploration_bonus': trial_params.get('reward_weight_exploration_bonus', 0.15),
                        'inactivity_penalty': trial_params.get('reward_weight_inactivity_penalty', -0.40),
                        'frequency_penalty': trial_params.get('reward_weight_frequency_penalty', -0.7),
                    ...
                    config_overrides = {
                        'strategy': {
                            ...
                            'reward_scaling_factor': trial_params.get('reward_scaling_factor', 200.0),
                            'inactivity_grace_period_steps': trial_params.get('inactivity_grace_period', 15),
                            'penalty_ramp_up_steps': trial_params.get('penalty_ramp_up_steps', 30),
                        }
                    }
                ...
                worker_logger.info(f"Worker {rank} environment initialized successfully.")
                return env
                
            except Exception as e:
                worker_logger.error(f"FATAL ERROR in worker {rank} during initialization: {e}", exc_info=True)
                raise
    ...
    def objective(self, trial) -> float:
        """
        CRITICAL FIX: Optuna objective that optimizes for raw trading performance (P&L)
        and explicitly penalizes exploitation of intrinsic rewards. This prevents the
        agent from learning the "penalty avoidance" loophole.
        """
        ...
        try:
            # Tighter, more focused hyperparameter ranges for stable optimization
            trial_params = {
                # PPO Core
                'learning_rate': trial.suggest_float('learning_rate', 5e-5, 5e-4, log=True),
                'n_steps': trial.suggest_categorical('n_steps', [1024, 2048, 4096]),
                'batch_size': trial.suggest_categorical('batch_size', [64, 128, 256]),
                'n_epochs': trial.suggest_int('n_epochs', 8, 18),
                'gamma': trial.suggest_float('gamma', 0.97, 0.995),
                'gae_lambda': trial.suggest_float('gae_lambda', 0.93, 0.98),
                'clip_range': trial.suggest_float('clip_range', 0.08, 0.2),
                'ent_coef': trial.suggest_float('ent_coef', 0.005, 0.04, log=True),
                
                # Transformer Architecture
                'transformer_d_model': trial.suggest_categorical('transformer_d_model', [64, 96, 128]),
                ...
                'reward_scaling_factor': trial.suggest_float('reward_scaling_factor', 150.0, 350.0),
                
                # Reward weights to prevent penalty avoidance
                'reward_weight_base_return': trial.suggest_float('reward_weight_base_return', 3.5, 5.5),
                'reward_weight_transaction_penalty': trial.suggest_float('reward_weight_transaction_penalty', -0.5, -0.05),
                'reward_weight_drawdown_penalty': trial.suggest_float('reward_weight_drawdown_penalty', -2.0, -0.8),
                'reward_weight_exploration_bonus': trial.suggest_float('reward_weight_exploration_bonus', 0.05, 0.25),
                'reward_weight_inactivity_penalty': trial.suggest_float('reward_weight_inactivity_penalty', -1.0, -0.2),
                'reward_weight_frequency_penalty': trial.suggest_float('reward_weight_frequency_penalty', -1.5, -0.5),
                'reward_weight_tiny_action_penalty': trial.suggest_float('reward_weight_tiny_action_penalty', -1.0, -0.2),
            }
            
            # Prune invalid PPO configurations early to save time
            ...
            eval_callback = PortfolioValueEvalCallback(
                ...
                render=False,
                n_eval_episodes=5,
                warn=False # Suppress warnings about not using default reward
            )
            
            perf_callback = PerformanceMonitoringCallback()
            hparam_metrics_to_log = ['eval/mean_portfolio_value', 'performance/total_return_raw']
            ...
            # --- New Objective Calculation Logic ---
            if perf_callback.portfolio_values and perf_callback.raw_rewards:
                ...
                # 1. Calculate Raw PnL Performance
                ...
                # Use a risk-adjusted metric for raw performance
                risk_adjusted_raw_return = total_return_raw / (max_drawdown + 0.01)
                
                # 2. Calculate Intrinsic Reward Exploitation
                ...
                # Calculate the fraction of the total reward that came from intrinsic sources
                exploit_fraction = 0.0
                if total_positive_reward > 0:
                    exploit_fraction = total_intrinsic_reward / total_positive_reward
                
                # 3. Penalize Exploitation
                # If more than 20% of reward is intrinsic, apply a harsh penalty.
                ...
                # Return a very bad score if training failed to produce metrics
                return -10.0
    ...
    def optimize(self, n_trials: int = 50, timeout: Optional[int] = None):
        ...
        try:
            ...
            study_name = f"zero1-ppo-optimization-{SETTINGS.environment.value}"
    
            logger.info(f"Optuna study '{study_name}' will be saved to: {db_path}")
            ...
            logger.info(f"Resuming study. Completed trials: {len(study.trials)}")
...
def train_model_fixed(optimization_trials: int = 20,
    ...
    try:
        ...
        if optimization_trials > 0 and OPTUNA_AVAILABLE:
            ...
            logger.info("Skipping optimization, using pre-tuned high-quality default parameters.")
            
            # --- START OF FIX #1: Update default params with new balanced rewards ---
            # These defaults are a high-quality starting point that discourages
            # the penalty avoidance loop and promotes stable learning.
            best_params = {
                'learning_rate': 0.0003, 'n_steps': 2048, 'batch_size': 128, 'n_epochs': 12,
                'gamma': 0.985, 'gae_lambda': 0.95, 'clip_range': 0.1, 'ent_coef': 0.01,
                'max_grad_norm': 0.7, 'transformer_d_model': 64, 'transformer_n_heads': 4,
                'transformer_dim_feedforward': 256, 'transformer_num_layers': 2,
                'expert_output_dim': 32, 'attention_features': 64, 'dropout_rate': 0.2,
                ...
                'reward_weight_base_return': 5.0,
                'reward_weight_transaction_penalty': -0.15,
                'reward_weight_drawdown_penalty': -2.0,
                'reward_weight_position_penalty': -0.05,
                'reward_weight_exploration_bonus': 0.1,
                'reward_weight_inactivity_penalty': -1.0,
                'reward_weight_frequency_penalty': -1.9,
                'reward_weight_tiny_action_penalty': -1.5,
