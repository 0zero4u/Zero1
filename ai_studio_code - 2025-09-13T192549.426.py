...
class FixedRewardCalculator:
    ...
    def calculate_immediate_reward(self, prev_value: float, curr_value: float,
        ...
        try:
            ...
            # The agent learned to exploit the normalizer by choosing actions that were
            # "less bad" than the historical average of all rewards (including penalties).
            # This led to positive final rewards for useless, penalized actions.
            #
            # THE FIX:
            # 1. Update the normalizer ONLY with the raw, extrinsic reward (PnL-based).
            #    This anchors the normalization to actual trading performance.
            # 2. Normalize ONLY the raw reward.
            # 3. Add the intrinsic (behavior-shaping) reward AFTER normalization. This
            #    treats penalties as a fixed "cost" that must be overcome by generating
            #    a high normalized raw reward.
            
            # Step 1 & 2: Update and normalize based on raw performance only.
            self.reward_normalizer.update(raw_reward)
            normalized_raw_reward = self.reward_normalizer.normalize(raw_reward)
            
            # Step 3: Add intrinsic penalties/bonuses as a fixed cost/benefit.
