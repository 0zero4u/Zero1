...
class EnhancedHierarchicalTradingEnvironment(gym.Env):
    ...
    def __init__(self, df_base_ohlc: pd.DataFrame, normalizer: Normalizer, config=None, 
        ...
        try:
            ...
            logger.info("Enhanced environment initialized successfully with reward horizon system.")
    ...
    def step(self, action: np.ndarray):
        ...
        try:
            ...
            info = {
                ...
                'consecutive_losses': self.consecutive_losses, 'trade_count': self.trade_count, 'win_rate': self.winning_trades / max(self.trade_count, 1),
                'reward_components': reward_components, 'dynamic_position_limit': dynamic_limit, 'transaction_cost': total_cost, 'leverage': self.leverage,
    ...
    def _get_single_step_observation(self, step_index) -> dict:
        try:
            ...
            for key_enum, lookback in self.strat_cfg.lookback_periods.items():
                key = key_enum.value
                if key in [FeatureKeys.PORTFOLIO_STATE.value, FeatureKeys.CONTEXT.value, FeatureKeys.PRECOMPUTED_FEATURES.value]: obs[key] = np.zeros(lookback, dtype=np.float32)
    ...
    def get_performance_metrics(self) -> Dict[str, float]:
        try:
            ...
            return {
                'total_return': total_return, 'volatility': volatility, 'max_drawdown': max_drawdown, 'sharpe_ratio': sharpe_ratio,
                'win_rate': win_rate, 'total_trades': self.trade_count, 'avg_reward': avg_reward, 'reward_volatility': reward_volatility,
                'consecutive_losses': self.consecutive_losses, 'final_portfolio_value': final_value, 'leverage': self.leverage,
                'reward_scaling_factor': self.reward_calculator.scaling_factor
            }
        except Exception as e:
...
def create_trading_environment(df_base_ohlc: pd.DataFrame, normalizer: Normalizer, 
    ...
    return EnhancedHierarchicalTradingEnvironment(df_base_ohlc=df_base_ohlc, normalizer=normalizer, config=config, leverage=leverage, reward_weights=reward_weights)
    ...
    try:
        ...
        for leverage in test_leverages:
            ...
            test_reward, components = reward_calc.calculate_enhanced_reward(
                ...
                portfolio_state={'drawdown': 0.05, 'margin_ratio': 0.8}, market_state={'regime': 'TRENDING_UP', 'volatility': 0.02}
            )
