...
class FixedRewardCalculator:
    ...
    def __init__(self, config, leverage: float = 10.0, reward_weights: Dict[str, float] = None):
        ...
        self.weights = reward_weights or {
            ...
            'frequency_penalty': -0.24,  # NEW: Penalty for hyperactivity
        }
...
class FixedHierarchicalTradingEnvironment(gymnasium.Env):
    ...
    def step(self, action: np.ndarray):
        ...
        try:
            ...
            # Check for insignificant trade attempts (agent wants to act, but size is too small)
            # This is a key metric for "thrashing" behavior.
            if (abs(action_signal) + action_size) > 0.01 and action_size < 0.01:
