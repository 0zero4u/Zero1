...
class EnhancedDataProcessor:
    ...
    def _create_bars_with_pandas(self, all_trade_files: List[str], additional_features: bool) -> pd.DataFrame:
        """Create bars using pandas with a memory-efficient, file-by-file approach."""
        logger.info(f"Loading {len(all_trade_files)} trade files with pandas (memory-efficient mode)...")
        ...
        try:
            ...
            for file_path in tqdm(all_trade_files, desc="Creating bars from files"):
                ...
                df = pd.read_parquet(file_path, columns=['timestamp', 'price', 'size', 'side'])
                ...
                # Resample this single file's data
                bars_chunk = df.resample(resample_freq).agg({
                    'price': ['first', 'max', 'min', 'last', 'count'],
                    'size': 'sum',
                    'price_volume': 'sum',
                    'buy_volume': 'sum',
            ...
            # Combine the much smaller, aggregated bar DataFrames
            combined_bars_df = pd.concat(all_bars)

            # IMPORTANT: Re-aggregate the combined data to correctly handle bars
            # that might span across two different files.
            final_bars_df = combined_bars_df.groupby(level=0).agg({
                ('price', 'first'): 'first',
                ('price', 'max'): 'max',
                ('price', 'min'): 'min',
                ('price', 'last'): 'last',
