...
from the updated engine.py to prevent ImportError during backtesting.
...
# --- START OF IMPORT FIX ---
# The environment class was renamed in engine.py to reflect the PPO reward fix.
# We use an alias to maintain compatibility with the rest of the code.
...
class AdvancedBacktester:
    ...
    def run_backtest(self, period: str = "out_of_sample",
        ...
        try:
            ...
            logger.info("Generating pre-computed features for backtesting...")
            features_df = generate_stateful_features_for_fitting(bars_df, self.cfg.strategy)
            ...
            env = EnhancedHierarchicalTradingEnvironment(
                ...
                precomputed_features=features_df  # <-- Pass the features here
            )
            # --- END OF FIX for Optimized Engine ---
...
class EnsembleBacktester:
    ...
    def run_ensemble_backtest(self) -> Dict:
        ...
        try:
            ...
            # --- START OF FIX for Optimized Engine ---
            logger.info("Generating pre-computed features for ensemble backtesting...")
            features_df = generate_stateful_features_for_fitting(bars_df, SETTINGS.strategy)
            # --- END OF FIX for Optimized Engine ---
            ...
            env = EnhancedHierarchicalTradingEnvironment(
                df_base_ohlc=bars_df, 
                normalizer=normalizer,
                precomputed_features=features_df
            )
            # --- END OF FIX for Optimized Engine ---
