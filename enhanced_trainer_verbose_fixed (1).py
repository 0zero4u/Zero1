        ...
        if self.n_calls % self.log_freq == 0:
            ...
            if raw_components:
                for k, v in raw_components.items(): 
                    self.logger.record(f'reward_raw/{k}', v)
            
            weighted_rewards = info.get('weighted_rewards', {})
            if weighted_rewards:
                for k, v in weighted_rewards.items(): 
                    self.logger.record(f'reward_weighted/{k}', v)
...
class EnhancedFixedTrainer:
    def __init__(self, base_config: Optional[Dict] = None, use_wandb: bool = False, enable_live_monitoring: bool = True):
        # FIXED: Replace deprecated .dict() with .model_dump()
        self.base_config = base_config or SETTINGS.model_dump()
    ...
    def optimize(self, n_trials: int = 50, timeout: Optional[int] = None):
        ...
        study = optuna.create_study(
            study_name=f"ppo-optimization-{SETTINGS.environment.value}", 
