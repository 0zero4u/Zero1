...
class HierarchicalTradingEnvironment(gym.Env):
    ...
    def step(self, action: np.ndarray):
        ...
        # If required margin exceeds available equity, cap the position size to what's affordable.
        if required_margin > initial_portfolio_value:
            # Calculate max affordable notional value based on equity and leverage.
            max_affordable_notional = initial_portfolio_value * self.strat_cfg.leverage
            # Cap the target asset quantity, preserving the original direction of the trade.
