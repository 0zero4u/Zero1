...
class FixedRewardCalculator:
    ...
    def __init__(self, config, leverage: float = 10.0, reward_weights: Dict[str, float] = None):
        ...
        # --- START OF FIX: Implement Selective Normalization ---
        # Define which signals are "core" (high variance, need normalization) vs "nudges" (low variance, use raw value)
        self.core_signals = {'base_return', 'drawdown_penalty', 'transaction_penalty'}
        # --- END OF FIX ---

        # Add attributes for self-normalization
        self.normalization_window = 1000
        self.reward_component_history = {key: deque(maxlen=self.normalization_window) for key in self.weights.keys()}
        self.reward_stats = {key: {'mean': 0.0, 'std': 1.0} for key in self.weights.keys()}
        self.epsilon = 1e-8
    ...
    def calculate_immediate_reward(self, prev_value: float, curr_value: float,
        ...
        try:
            ...
            # --- START OF FIX: Selective Normalization to Restore Hierarchy ---
            all_raw_components = {**raw_components, **intrinsic_components}

            # Update history and stats for all components for logging purposes
            for component, value in all_raw_components.items():
                if component in self.reward_component_history:
                    self.reward_component_history[component].append(value)
                    history = self.reward_component_history[component]
                    if len(history) > 50:
                        self.reward_stats[component]['mean'] = np.mean(history)
                        self.reward_stats[component]['std'] = np.std(history) + self.epsilon

            # Calculate the final reward using selective normalization
            total_reward = 0.0
            normalized_components_log = {}

            for component, value in all_raw_components.items():
                if component in self.core_signals:
                    # NORMALIZE Core Signals (PnL, Drawdown, Costs)
                    stats = self.reward_stats.get(component, {'mean': 0.0, 'std': 1.0})
                    normalized_value = (value - stats['mean']) / stats['std']
                    total_reward += normalized_value
                    normalized_components_log[f"{component}_norm"] = normalized_value
                else:
                    # USE RAW Nudge Signals (Exploration, Inactivity, etc.)
                    total_reward += value
                    normalized_components_log[f"{component}_raw"] = value # Log the raw value used

            # Clip the final combined reward
            total_reward = np.clip(total_reward, -3.0, 3.0)
            # --- END OF FIX ---
            
            raw_reward_unnormalized = sum(raw_components.values())
            intrinsic_reward_unnormalized = sum(intrinsic_components.values())
            
            self.reward_history.append(total_reward)
            
            final_components_log = {**all_raw_components, **normalized_components_log}

            return total_reward, raw_reward_unnormalized, intrinsic_reward_unnormalized, final_components_log
