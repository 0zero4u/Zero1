...
class HierarchicalTradingEnvironment(gym.Env):
    ...
    def _get_single_step_observation(self, step_index) -> dict:
        ...
        try:
            ...
            # Special handling for dist_vwap_3m, which needs a resampled dataframe
            df_3m = self.timeframes['3T']
            row_3m = df_3m.loc[df_3m.index.get_loc(current_timestamp, method='ffill')]
            dist_vwap_3m = (row_3m['close'] - row_3m['vwap']) / (row_3m['vwap'] + 1e-9) if 'vwap' in row_3m and row_3m['vwap'] != 0 else 0.0
            
            # Create the final vector of precomputed features
            precomputed_vector = current_bar_features[self.strat_cfg.precomputed_feature_keys].fillna(0.0).values.astype(np.float32)
            # Find the index for dist_vwap_3m and insert the value
            try:
                vwap_idx = self.strat_cfg.precomputed_feature_keys.index('dist_vwap_3m')
                precomputed_vector[vwap_idx] = dist_vwap_3m
            except ValueError:
                pass # 'dist_vwap_3m' not in the list
