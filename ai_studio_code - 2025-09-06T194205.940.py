...
def generate_stateful_features_for_fitting(bars_df: pd.DataFrame, strat_cfg: StrategyConfig) -> pd.DataFrame:
    ...
    for calc_cfg in tqdm(strat_cfg.stateful_calculators, desc="Calculating features"):
        ...
        # --- START OF FIX ---
        # Logic to map generated column names to the configured output_keys.
        if len(result_df.columns) == len(calc_cfg.output_keys):
            # Assume the order of generated columns matches the order in output_keys.
            # This is crucial for multi-output calculators like StatefulSRDistances.
            rename_map = dict(zip(result_df.columns, calc_cfg.output_keys))
            result_df = result_df.rename(columns=rename_map)
            logger.debug(f"Mapped generated columns for {calc_cfg.name} to {list(result_df.columns)}")
        else:
            logger.warning(f"Mismatch between generated columns ({len(result_df.columns)}) and output_keys ({len(calc_cfg.output_keys)}) for {calc_cfg.name}. Skipping renaming.")
        # --- END OF FIX ---

        # Reindex and merge each valid column into the main features DataFrame
        for col in result_df.columns:
            # After renaming, the column name should be in output_keys
            if col in calc_cfg.output_keys:
                features_df[col] = result_df[col].reindex(features_df.index, method='ffill')
            else:
                logger.warning(f"Generated column '{col}' for '{calc_cfg.name}' is not in its configured output_keys and was ignored.")
