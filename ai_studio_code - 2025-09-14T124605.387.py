...
from enhanced_engine_verbose import FixedHierarchicalTradingEnvironment # Use the new engine
...
class PortfolioValueEvalCallback(EvalCallback):
    """
    Custom EvalCallback to evaluate based on final portfolio value (raw P&L).
    This callback is already robust and requires no changes.
    """
    ...
    def _on_step(self) -> bool:
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            for _ in range(self.n_eval_episodes):
                obs, done, episode_reward = self.eval_env.reset(), False, 0.0
                while not done:
...
class PerformanceMonitoringCallback(BaseCallback):
    """
    FIXED: Performance monitoring callback compatible with the new reward architecture.
    """
    ...
    def _on_step(self) -> bool:
        if self.n_calls % 700 == 0 and self.locals.get('infos'):
            ...
            # --- FIX: Log the new reward component structures ---
            # Log raw components for distribution analysis
            ...
            # Log final weighted components to see what drives agent behavior
...
class EnhancedFixedTrainer:
    ...
    def _make_env(self, rank: int, seed: int = 0, trial_params: Optional[Dict] = None) -> Callable:
        def _init():
            ...
            if trial_params:
                ...
                # --- FIX: Use reward weight keys that match the new RewardManager ---
                reward_weights = {
                    'base_return': trial_params.get('reward_weight_base_return', 1.0),
                    'transaction_penalty': trial_params.get('reward_weight_transaction_penalty', -0.6),
                    'drawdown_penalty': trial_params.get('reward_weight_drawdown_penalty', -1.0),
                    'frequency_penalty': trial_params.get('reward_weight_frequency_penalty', -0.5),
                    'tiny_action_penalty': trial_params.get('reward_weight_tiny_action_penalty', -0.1),
                }
    ...
    def objective(self, trial) -> float:
        """
        FIXED: Optuna objective that optimizes directly for portfolio value.
        """
        if not OPTUNA_AVAILABLE: raise RuntimeError("Optuna not available.")
        
        try:
            # --- FIX: Update trial params to match the new reward weights ---
            trial_params = {
                ...
                # Reward weights for the new RewardManager
                'reward_weight_base_return': trial.suggest_float('reward_weight_base_return', 0.8, 1.2),
                'reward_weight_transaction_penalty': trial.suggest_float('reward_weight_transaction_penalty', -1.0, -0.2),
                'reward_weight_drawdown_penalty': trial.suggest_float('reward_weight_drawdown_penalty', -1.5, -0.5),
                'reward_weight_frequency_penalty': trial.suggest_float('reward_weight_frequency_penalty', -0.8, -0.2),
            }
            ...
            vec_env = SubprocVecEnv([self.make_env(i, trial.number, trial_params=trial_params) for i in range(self.num_cpu)])
            ...
            eval_env = DummyVecEnv([self.make_env(rank=0, seed=123, trial_params=trial_params)])
            ...
            if self.use_wandb:
                callbacks.append(WandbCallback("crypto_trading_hardened", experiment_name, trial_params))
            
            model.learn(total_timesteps=30720, callback=callbacks, progress_bar=False)
            
            # --- FIX: Use the reliable metric from the eval callback as the objective value ---
            objective_value = eval_callback.best_mean_portfolio_value - SETTINGS.strategy.initial_balance
    ...
    def train_best_model(self, best_params: Dict, final_training_steps: int) -> PPO:
        ...
        vec_env = SubprocVecEnv([self.make_env(i, seed=42, trial_params=best_params) for i in range(self.num_cpu)])
        ...
        if self.use_wandb:
            callbacks.append(WandbCallback("crypto_trading_final_hardened", f"final_train_{datetime.now().strftime('%Y%m%d_%H%M%S')}", best_params))
...
def train_model_fixed(optimization_trials: int = 20,
    ...
    try:
        ...
        if optimization_trials > 0 and OPTUNA_AVAILABLE:
            ...
            # --- FIX: Update default params with new balanced reward weights ---
            best_params = {
                ...
                'reward_weight_transaction_penalty': -0.6,
                'reward_weight_drawdown_penalty': -1.0,
                'reward_weight_frequency_penalty': -0.5,
                'reward_weight_tiny_action_penalty': -0.1,
            }
        
        trainer.train_best_model(best_params, final_training_steps)
        logger.info(f"🎉 Hardened training completed! Model saved to: {trainer.last_saved_model_path}")
        return trainer.last_saved_model_path
        
    except Exception as e:
        logger.error(f"Hardened training pipeline failed: {e}", exc_info=True)
        raise
