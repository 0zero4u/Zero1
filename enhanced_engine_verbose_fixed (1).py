...
class RewardManager:
    ...
    def _calculate_raw_components(self, **kwargs) -> Dict[str, float]:
        ...
        # --- START OF MODIFICATION: Add Action Clarity Bonus ---
        if action_size > 0.25:
            components['action_clarity'] = 0.1
        else:
    ...
    def _transform_to_ratios(self, raw_components: Dict[str, float], **kwargs) -> Dict[str, float]:
        ...
        # --- START OF MODIFICATION: Pass through Action Clarity Bonus ---
...
class FixedHierarchicalTradingEnvironment(gymnasium.Env):
    ...
    def __init__(self, df_base_ohlc: pd.DataFrame, normalizer: Normalizer, config=None,
        ...
        try:
            ...
            # --- START OF MODIFICATION: Add new default reward weight ---
            default_reward_weights = {
                ...
                'inactivity': 0.2,
                'tiny_action': 0.3,
                'action_clarity': 0.1,  # Added new bonus component
            }
            # --- END OF MODIFICATION ---
    ...
    def _get_single_step_observation(self, step_index) -> dict:
        try:
            ...
            for key_enum, lookback in self.strat_cfg.lookback_periods.items():
                ...
                if key.startswith('ohlc') or key.startswith('ohlcv'):
                ...
                if len(window_data) < lookback:
                    pad_shape = (lookback - len(window_data),) + window_data.shape[1:] if hasattr(window_data, 'shape') and len(window_data.shape) > 0 else (lookback - len(window_data),)
                    window_data = np.pad(window_data, [(lookback - len(window_data), 0)] + [(0,0)]*(window_data.ndim-1), 'edge')
