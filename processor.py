...
class EnhancedDataProcessor:
    ...
    def create_enhanced_bars_from_trades(self, period_name: str, 
        ...
        logger.info(f"✅ Generated {len(bars_df):,} bars from "
                   f"{bars_df.index.min()} to {bars_df.index.max()}")
    ...
    def _add_enhanced_features(self, ohlcv_df: pd.DataFrame) -> pd.DataFrame:
        ...
        ohlcv_df['trade_intensity'] = ohlcv_df['trade_count'] / ohlcv_df['volume']
    ...
    def _intelligent_gap_filling(self, df: pd.DataFrame) -> pd.DataFrame:
        """Intelligent gap filling for missing bars."""
        
        # Forward fill basic OHLCV values for small gaps (< 5 minutes)
        time_diff = df.index.to_series().diff()
        small_gaps = time_diff <= pd.Timedelta(minutes=5)
        
        for col in ['open', 'high', 'low', 'close']:
            df.loc[small_gaps, col] = df[col].fillna(method='ffill')
        
        # Set volume to 0 for gaps
        ...
        for col in numeric_cols:
            if col not in ['volume', 'trade_count']:
    ...
    def create_feature_store(self, period_name: str) -> Dict[str, pd.DataFrame]:
        """Create a comprehensive feature store for the period."""
        
        logger.info(f"🏪 Creating feature store for period: {period_name}")
        
        # Load base OHLCV data
        bars_df = self.create_enhanced_bars_from_trades(period_name)
        
        if bars_df.empty:
            logger.warning("No bars data available for feature store")
            return {}
        
        # Load funding rate data
        funding_df = self.load_and_prepare_funding_data_enhanced()
        ...
        # Create additional timeframe aggregations
        timeframes = ['5T', '15T', '1H', '4H', '1D']
        
        for tf in timeframes:
            try:
                # Resample to higher timeframes
                bars_resampled = self._resample_bars_to_timeframe(bars_df, tf)
                feature_store[f'ohlcv_{tf}'] = bars_resampled
            except Exception as e:
                logger.warning(f"Failed to create {tf} timeframe: {e}")
        
        logger.info(f"✅ Feature store created with {len(feature_store)} datasets")
        return feature_store

    def _resample_bars_to_timeframe(self, bars_df: pd.DataFrame, timeframe: str) -> pd.DataFrame:
        ...
        # Add additional column aggregation rules
        additional_cols = ['avg_trade_size', 'trade_size_std', 'typical_price', 'price_range']
        for col in additional_cols:
            if col in bars_indexed.columns:
                agg_rules[col] = 'mean'
        
        # Resample
        resampled = bars_indexed.resample(timeframe).agg(agg_rules)
...
class DataAnalyzer:
    ...
    def analyze_trading_patterns(self, period_name: str) -> Dict:
        """Analyze trading patterns and market microstructure."""
        
        logger.info(f"📈 Analyzing trading patterns for {period_name}")
        
        # Load trade data
        files = self.processor.get_files_for_period(period_name, "processed_trades")
        
        if not files:
            return {}
        
        # Sample analysis on a subset of files for performance
        ...
        daily_volume = trades_df.groupby('day_of_week')['size'].sum()
        ...
        if 'side' in trades_df.columns:
            ...
            analysis['side_analysis'] = side_analysis.to_dict()
...
if __name__ == "__main__":
    ...
    # Create feature store
    feature_store = processor.create_feature_store("in_sample")
