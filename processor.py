...
class EnhancedDataProcessor:
    ...
    def _create_bars_with_pandas(self, all_trade_files: List[str], additional_features: bool) -> pd.DataFrame:
        ...
        try:
            ...
            # --- START: REMOVED CONTRADICTORY LOGIC ---
            # The pre-computation of `dist_vwap_3m` has been removed from here.
            # This feature will now be correctly calculated by the StatefulVWAPDistance
            # calculator during the environment simulation, respecting the declarative
            # configuration in config.py.
            # --- END: REMOVED CONTRADICTORY LOGIC ---
...
def generate_stateful_features_for_fitting(bars_df: pd.DataFrame, strat_cfg: StrategyConfig) -> pd.DataFrame:
    ...
    for calc_cfg in tqdm(strat_cfg.stateful_calculators, desc="Calculating features"):
        ...
        # --- START OF REFINEMENT ---
            ...
            # This calculator operates on the base timeframe (e.g., 20S)
            # but needs both price and volume.
            period = calc_cfg.params['period']
            price_series = base_df_indexed['close']
            volume_series = base_df_indexed['volume']

            # Calculate price-volume product
            pv = price_series * volume_series

            # Calculate rolling sums for VWAP
            ...
            # Calculate the distance from the current price to the VWAP
            dist = (price_series - vwap) / (vwap + 1e-9)
            
            # Since this is already on the base index, no reindexing is needed.
            features_df[calc_cfg.output_keys[0]] = dist
        # --- END OF REFINEMENT ---
