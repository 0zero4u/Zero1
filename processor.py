...
class EnhancedDataProcessor:
    ...
    def _create_bars_with_pandas(self, all_trade_files: List[str], additional_features: bool) -> pd.DataFrame:
        """FIXED: Create bars using an efficient, chunked pandas approach that avoids quadratic slowdown."""
        logger.info(f"Loading {len(all_trade_files)} trade files with pandas (efficient chunking mode)...")
        ...
        try:
            ...
            all_bars_chunks = []
            for file_path in tqdm(all_trade_files, desc="Reading trade chunks"):
                ...
                for batch in pf.iter_batches(batch_size=2_000_000):
                    ...
                    all_bars_chunks.append(bars_chunk)

            if not all_bars_chunks:
                logger.warning("No bar data was generated from any files.")
                return pd.DataFrame()

            # --- START OF FIX: Perform aggregation only ONCE after collecting all chunks ---
            logger.info(f"Combining {len(all_bars_chunks)} chunks for final aggregation...")
            combined_chunks = pd.concat(all_bars_chunks)
            ...
            final_bars_df = combined_chunks.groupby(level=0).agg(regroup_rules)
            # --- END OF FIX ---
