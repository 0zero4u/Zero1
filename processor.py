    ...
    def _create_bars_with_pandas(self, all_trade_files: List[str], additional_features: bool) -> pd.DataFrame:
        ...
        try:
            ...
            for file_path in tqdm(all_trade_files, desc="Creating bars from files"):
                ...
                # Incrementally combine and re-aggregate
                combined = pd.concat([final_bars_df, bars_chunk])
                
                # This groupby ensures that bars spanning file boundaries are correctly merged
                final_bars_df = combined.groupby(level=0).agg({
                    ('price', 'first'): 'first',
                    ('price', 'max'): 'max',
                    ('price', 'min'): 'min',
                    ('price', 'last'): 'last',
