...
class ConstraintManager:
    ...
    def update_lambdas(self, costs_batch: torch.Tensor, constraint_keys: List[str]):
        ...
        for key, log_lambda in self.log_lambdas.items():
            wandb.log({f"lambda/{key}": torch.exp(log_lambda).item()})
...
class LagrangianTQC(TQC):
    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            super()._train_on_batch(replay_data)

    def _train_on_batch(self, replay_data: ReplayBuffer):
        """Helper method to perform a single gradient update step on a batch of data."""
        with torch.no_grad():
            next_actions, next_log_prob = self.actor.action_log_prob(replay_data.next_observations)
            next_quantiles = torch.cat(self.critic.forward(replay_data.next_observations, next_actions), dim=1)
            ...
            truncated_next_quantiles = sorted_next_quantiles[:, : self.critic.n_quantiles - top_quantiles_to_drop]
        ...
        current_quantiles_list = self.critic.forward(replay_data.observations, replay_data.actions)
        critic_loss = 0.0
        ...
        q_values_pi = torch.cat(self.critic.forward(replay_data.observations, actions), dim=1).mean(dim=1, keepdim=True)
...
# Callbacks remain largely the same, but we can log more specific things if needed.
class PortfolioValueEvalCallback(EvalCallback): # (Unchanged)
    # ... (code is identical to original, no need to repeat)
...
class HParamCallback(BaseCallback): # (Unchanged)
    # ... (code is identical to original, no need to repeat)
...
class RolloutInfoCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.n_calls % self.log_freq == 0:
            ...
            # Also log the current lambda values from the model
...
class EpisodeStatsCallback(BaseCallback): # (Unchanged, but one line corrected from original file)
    # ... (code is identical to original, no need to repeat)
...
class WandbCallback(BaseCallback): # (Unchanged)
    # ... (code is identical to original, no need to repeat)
...
class EnhancedFixedTrainer:
    def __init__(self, base_config: Optional[Dict] = None, use_wandb: bool = False, enable_live_monitoring: bool = True):
        ...
        # --- The ConstraintManager is created once in the main trainer process ---
    ...
    def _make_env(self, rank: int, seed: int = 0, trial_params: Optional[Dict] = None, initial_lambdas: Optional[Dict] = None) -> Callable:
        def _init():
            ...
            constraint_targets = {
                'drawdown': 0.15,
                'trade_cost_pct': 0.0005, # Target 0.05% cost per step on average
                'thrashing_rate': 0.1,   # Target <10% of trades being flips
            }
            if trial_params:
            ...
            env = FixedHierarchicalTradingEnvironment(
                ...
                initial_lambdas=initial_lambdas or {k: 1.0 for k in self.constraint_keys}, # Pass current lambdas
    ...
    def create_lagrangian_model(self, trial_params: Dict, vec_env: VecEnv) -> LagrangianTQC:
        """Creates the ConstraintManager and the LagrangianTQC model."""
        constraint_targets = {
        ...
        # --- Use LagrangianTQC instead of SAC/TQC ---
    ...
    def objective(self, trial) -> float:
        if not OPTUNA_AVAILABLE: raise RuntimeError("Optuna not a
