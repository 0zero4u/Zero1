...
FIX #5: CRITICAL - The exploration_bonus is now scaled by trade size.
FIX #6: CRITICAL - Refactored position sizing logic to decouple signal from size,
        fixing the root cause of "phantom profits" and unstable state.
FIX #6: CRITICAL - Implemented proper entry price averaging logic to correctly handle
        adding to or partially closing positions.
    ...
    def calculate_immediate_reward(self, prev_value: float, curr_value: float,
        ...
        try:
            ...
            if position_size_action > 0.01:
                ...
                exploration_reward = (action_magnitude * position_size_action) * volatility_scaler * (1 - position_confidence)
    ...
    def step(self, action: np.ndarray):
        ...
        try:
            ...
            if action_size < 0.01:
                ...
                # --- START OF CRITICAL FIX (FIX #6): Decouple Signal from Size ---
                # The previous formula `... * action_signal * action_size` created phantom
                # profits and unstable behavior.
                #
                # THE FIX:
                # Use signal for direction (sign) and size for magnitude. This is the
                # standard, stable way to handle position sizing.
                ...
                # Magnitude is determined by action_size and the dynamic limit
                target_notional_magnitude = initial_portfolio_value * action_size * dynamic_limit
                
                # Direction is determined by the sign of action_signal
                target_notional = np.sign(action_signal) * target_notional_magnitude
                # --- END OF CRITICAL FIX (FIX #6) ---
            ...
            # --- START OF CRITICAL FIX (FIX #6): Proper Entry Price Averaging ---
            # The previous logic `self.entry_price = current_price` was incorrect for
            # adding to or reducing positions.
            #
            # THE FIX:
            # Implement weighted average price logic for all position changes.
            if abs(trade_quantity) > 1e-9:
                # Position was opened or flipped
                if abs(previous_asset_held) < 1e-9 or np.sign(trade_quantity) != np.sign(previous_asset_held):
                    self.entry_price = current_price
                # Position was increased (averaging down/up)
                elif abs(trade_quantity) > 1e-9 and np.sign(trade_quantity) == np.sign(previous_asset_held):
                    # Weighted average of the old position and the new trade
                    total_cost_of_assets = (previous_asset_held * self.entry_price) + (trade_quantity * current_price)
                    self.entry_price = total_cost_of_assets / (previous_asset_held + trade_quantity)
                # If reducing a position, the entry price of the remaining assets does not change.
