...
FIX #2: Corrected normalization logic to process the TOTAL reward, stabilizing the learning signal.
...
class FixedRewardCalculator:
    ...
    def calculate_immediate_reward(self, prev_value: float, curr_value: float,
        ...
        try:
            ...
            # --- START OF FIX #2: Corrected Normalization Logic ---
            # 1. Calculate the final, un-normalized reward by combining all components.
            total_unnormalized_reward = raw_reward + intrinsic_reward
            
            # 2. Update the normalizer with this complete reward signal.
            self.reward_normalizer.update(total_unnormalized_reward)
            
            # 3. Normalize the total value. This is the final reward sent to the agent.
            total_reward = self.reward_normalizer.normalize(total_unnormalized_reward)
            # --- END OF FIX #2 ---
