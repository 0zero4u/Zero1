...
class RewardManager:
    ...
    def _transform_to_ratios(self, raw_components: Dict[str, float], **kwargs) -> Dict[str, float]:
        ...
        # --- CORRECT: Scale financial components that were converted to a return percentage ---
        ...
        # --- CORRECTED: Remove incorrect scaling from penalties that are already ratios ---
        ...
        if 'hesitation' in raw_components:
            ratios['hesitation'] = -raw_components['hesitation']
...
class FixedHierarchicalTradingEnvironment(gymnasium.Env):
    ...
    def step(self, action: np.ndarray):
        try:
            ...
            # --- START OF FIX: Corrected indentation for flip calculation logic ---
            # This logic should only run WHEN a trade is executed to avoid a NameError
            # and to correctly attribute flips to trading actions.
