    ...
    def calculate_final_reward(self, **kwargs) -> Tuple[float, Dict[str, Any]]:
        ...
        for key, weight in self.weights.items():
            if key in transformed_ratios and transformed_ratios[key] != 0:
                ...
                # --- START OF CORRECTED FIX (Properly indented) ---
                # This block is now correctly indented inside the loop.
                # It will run for every component (pnl, drawdown, etc.) on every step.
                
                # Rule 1: Handle designated penalties. A penalty can never become a
                # positive reward, even if it's "less bad" than average.
                if key in self.penalty_components:
                    norm_value = min(0.0, norm_value)
                # Rule 2: Explicitly handle the PnL component. If the underlying PnL
                # was negative, the final reward component for PnL can never be positive.
                # This is the critical fix for the observed gaming behavior.
