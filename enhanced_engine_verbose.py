"""
ENHANCED FIXED ENGINE (POST-ANALYSIS): Now with Live Trade Monitoring

This version adds verbose mode for real-time trade monitoring during training.
Only worker #0 will print detailed trade information to avoid output spam.
"""
...
class FixedRewardCalculator:
    ...
    3. MARKET REGIMES: Added multipliers to scale rewards based on market conditions.
    4. ACTION REGULATION: Introduced a penalty for hyperactivity to prevent excessive trading.
    5. BOUNDED COMPONENTS: Added stricter clipping on risk-adjusted rewards.
    """
    ...
    def __init__(self, config, leverage: float = 10.0, reward_weights: Dict[str, float] = None):
        ...
        logger.info("APPLIED ANALYSIS FIXES: Dynamic Rewards, Rebalanced Weights, Market Regimes, Action Regulation.")

    # NEW: Helper method for market regime multipliers
    def _get_market_multipliers(self, market_regime: str) -> Dict[str, float]:
        """Get reward multipliers based on the current market regime."""
        base_multipliers = {
        ...
        return base_multipliers.get(market_regime, {})
    ...
    def calculate_immediate_reward(self, prev_value: float, curr_value: float,
        ...
        try:
            components = {}
            market_regime = market_state.get('regime', 'UNCERTAIN')
            market_volatility = market_state.get('volatility', 0.01)
            
            # Get market regime multipliers
            multipliers = self._get_market_multipliers(market_regime)
            ...
            if len(self.return_buffer) >= 20:
                ...
                if volatility > 1e-6:
                    ...
                    # REBALANCED: Tighter clip on risk-adjusted score
                    risk_adjusted_score = np.clip(np.tanh(sharpe_like), -0.1, 0.1)
                else:
            ...
            if action_magnitude > 0.006:
                position_confidence = abs(action[0])  # Proxy for confidence in direction
                # Scale bonus by volatility (more reward in volatile markets) and uncertainty (less confidence)
            ...
            # DYNAMIC: Action reward scaled by market opportunity (volatility)
            if action_magnitude > 0.001:
                # Use volatility as a proxy for market opportunity
                opportunity_score = np.clip(market_volatility * 100, 0.5, 1.5)
                action_reward = min(0.08, action_magnitude * opportunity_score)
            ...
            for component, multiplier in multipliers.items():
                if component in components:
                    components[component] *= multiplier
...
class FixedHierarchicalTradingEnvironment(gym.Env):
    ...
    NOW WITH LIVE TRADE MONITORING for debugging and intuition building.
    """
    
    def __init__(self, df_base_ohlc: pd.DataFrame, normalizer: Normalizer, config=None,
                 leverage: float = None, reward_weights: Dict[str, float] = None,
                 precomputed_features: Optional[pd.DataFrame] = None,
                 worker_id: int = 0):  # NEW: Worker ID for verbose mode
        super().__init__()
        
        # --- NEW: Verbose Mode Setup ---
        self.worker_id = worker_id
        self.verbose = self.worker_id == 0  # Only worker 0 will be verbose
        ...
        # Track previous position for trade detection
        self.previous_asset_held = 0.0
        ...
        if self.verbose:
            logger.info("--- Initializing FIXED High-Performance Trading Environment (VERBOSE MODE) ---")
            logger.info(f" -> FIXED: Prevents turtling and reward imbalance")
            logger.info(f" -> FIXED: Encourages exploration and learning")
            logger.info(f" -> NEW: Live trade monitoring enabled for Worker {self.worker_id}")
        ...
        try:
            ...
            if self.verbose:
                logger.info("✅ FIXED environment initialized successfully with LIVE MONITORING.")
    ...
    def _print_trade_info(self, trade_type: str, action: np.ndarray, current_price: float, 
        ...
        """Prints a detailed summary of a trade if in verbose mode."""
        if not self.verbose:
            return  # Do nothing if this isn't worker #0
        ...
        # Market Regime
        regime_color = {
            "HIGH_VOLATILITY": self.COLOR_RED,
            "TRENDING_UP": self.COLOR_GREEN,
            "TRENDING_DOWN": self.COLOR_RED,
            "LOW_VOLATILITY": self.COLOR_BLUE,
            "SIDEWAYS": self.COLOR_YELLOW,
            "UNCERTAIN": self.COLOR_MAGENTA
        ...
        # Top reward components (show only significant ones)
        significant_components = {k: v for k, v in reward_components.items() 
                                if abs(v) > 0.001 and k != 'error'}
    ...
    def _print_episode_start(self):
        ...
        self.trade_counter = 0  # Reset trade counter for new episode

    def step(self, action: np.ndarray):
        """
        FIXED: Step function with improved reward calculation, action encouragement,
        and LIVE TRADE MONITORING.
        """
        try:
            ...
            # Store previous position for trade detection
            ...
            # NEW: Detect and print trades in verbose mode
            position_changed = abs(self.asset_held - previous_asset_held) > 1e-9
            if position_changed and self.verbose:
                # Determine trade type
                if abs(self.asset_held) > abs(previous_asset_held):
                    if self.asset_held > 0:
                        trade_type = f"{self.COLOR_GREEN}🚀 OPEN/ADD LONG{self.COLOR_RESET}"
                    else:
                        trade_type = f"{self.COLOR_RED}🔻 OPEN/ADD SHORT{self.COLOR_RESET}"
                elif abs(self.asset_held) < abs(previous_asset_held):
                    if previous_asset_held > 0:
                        trade_type = f"{self.COLOR_YELLOW}📈 CLOSE/REDUCE LONG{self.COLOR_RESET}"
                    else:
                        trade_type = f"{self.COLOR_YELLOW}📉 CLOSE/REDUCE SHORT{self.COLOR_RESET}"
                else:
                    if self.asset_held > 0:
                        trade_type = f"{self.COLOR_BLUE}🔄 FLIP TO LONG{self.COLOR_RESET}"
                    else:
                        trade_type = f"{self.COLOR_BLUE}🔄 FLIP TO SHORT{self.COLOR_RESET}"
    ...
    def get_performance_metrics(self) -> Dict[str, float]:
        ...
        try:
            ...
            base_metrics = {
                ...
                'verbose_mode': self.verbose, 'worker_id': self.worker_id
            }
            ...
            return {'leverage': self.leverage, 'fixed_reward_system': True, 'verbose_mode': self.verbose}
