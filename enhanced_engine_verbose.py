...
CRITICAL FIX #3 (REFINED DIAGNOSIS): The 'exploration_bonus' was identified as a source of
                 reward hacking. The agent could perform tiny, inconsequential trades to
                 "farm" this bonus. The fix gates the exploration bonus, requiring a
                 meaningful change in the portfolio's risk exposure before any bonus is
                 awarded. This closes the loophole and forces the agent to seek profit
                 instead of chasing easily exploitable intrinsic rewards.
--- END OF CRITICAL FIXES ---

--- START OF APPLIED FIXES (Based on Analysis) ---
FIX #1: Rebalanced reward weights to prioritize profit and exploration over penalty avoidance.
FIX #2: CRITICAL - Corrected normalization logic to process only the RAW (extrinsic) reward,
        preventing the agent from "hacking" the reward function by exploiting penalties.
FIX #4: Made the action frequency penalty adaptive to market volatility.
FIX #5: CRITICAL - The frequency penalty is now applied continuously after a small warmup,
        instead of as a single large shock after 100 steps. This provides immediate,
        stable feedback to the agent and fixes the credit assignment problem.
FIX #5: Replaced non-linear penalty scaling with smoother linear/quadratic functions.
...
class RewardNormalizer:
    ...
    def update(self, reward: float):
        """Update the running statistics with a new reward value."""
        self.count += 1
    ...
    def normalize(self, reward: float) -> float:
        """Normalize a reward using the current running statistics."""
        return (reward - self.mean) / (self.std + self.epsilon)
# --- END OF MODIFICATION (Phase 2) ---
...
class EnhancedRiskManager:
    ...
    # --- START OF MODIFICATION (Phase 1) ---
    def get_volatility_percentile(self, current_volatility: float) -> float:
        """Calculates the percentile of the current volatility within its recent history."""
        if len(self.volatility_buffer) < 20:
            return 0.5 # Return a neutral 50th percentile if not enough history
        
        # Using scipy.stats is robust and handles edge cases.
        return stats.percentileofscore(list(self.volatility_buffer), current_volatility) / 100.0
    # --- END OF MODIFICATION (Phase 1) ---
...
class FixedRewardCalculator:
    ...
    def __init__(self, config, leverage: float = 10.0, reward_weights: Dict[str, float] = None):
        ...
        self.optimal_action_threshold = 0.30 # Base threshold, will be made dynamic
        self.action_frequency_buffer = deque(maxlen=self.action_frequency_window)
        
        # --- START OF FIX #1: Rebalance reward weights ---
        ...
        # Add the reward normalizer instance
        ...
        logger.info(f"APPLIED ANALYSIS FIXES: Rebalanced Weights, Adaptive Penalties, Corrected Normalization.")
    ...
    def calculate_immediate_reward(self, prev_value: float, curr_value: float,
                                 ...
                                 is_insignificant_action: bool,
                                 consecutive_inactive_steps: int = 0,
                                 trade_notional: float = 0.0,
                                 previous_notional: float = 0.0) -> Tuple[float, float, float, Dict]:
        """
        ...
        try:
            ...
            if current_drawdown > 0.08:
                ...
                # --- START OF FIX #5: Use linear scaling for drawdown penalty ---
            ...
            if inactivity_penalty_weight < 0 and consecutive_inactive_steps > self.inactivity_grace_period:
                ...
                # --- START OF FIX #5: Use linear scaling for inactivity penalty ---
            ...
            # --- START OF CRITICAL FIX #3: Gated, Risk-Adjusted Exploration Bonus ---
            # This logic prevents the agent from "farming" the exploration bonus with tiny,
            # meaningless trades. A bonus is only awarded if the agent makes a meaningful
            # change to its risk exposure.

            # Calculate the change in risk exposure as a fraction of the portfolio
            change_in_notional = abs(trade_notional - previous_notional)
            risk_commitment_pct = change_in_notional / max(prev_value, 1e-6)

            # Define a minimum threshold for what constitutes a meaningful change in risk
            MINIMUM_RISK_COMMITMENT_PCT = 0.001  # e.g., 0.1% of portfolio value

            if risk_commitment_pct > MINIMUM_RISK_COMMITMENT_PCT:
                action_magnitude = abs(action[0]) + abs(action[1])
                position_confidence = abs(action[0])
                
                # Scale the bonus by how much risk was taken on.
                commitment_scaler = np.clip(risk_commitment_pct / (MINIMUM_RISK_COMMITMENT_PCT * 10), 0.5, 1.5)
                volatility_scaler = np.clip(market_volatility * 50, 0.5, 2.0)
                
                exploration_reward = (action_magnitude * 0.5) * volatility_scaler * (1 - position_confidence) * commitment_scaler
                intrinsic_components['exploration_bonus'] = min(0.1, exploration_reward) * self.weights['exploration_bonus']
            else:
                # If the trade is not meaningful, no exploration bonus is given.
            ...
            # --- START OF CRITICAL FIX (FIX #5): Apply Frequency Penalty Continuously ---
            # The previous logic only applied the penalty on the 100th step, causing a
            # "reward shock" and preventing the agent from learning.
            #
            # THE FIX:
            # Apply the penalty on every step after a minimum lookback period (e.g., 20 steps)
            # is established. This provides immediate and smooth feedback.
            
            MIN_LOOKBACK_FOR_FREQ_PENALTY = 20
            if len(self.action_frequency_buffer) > MIN_LOOKBACK_FOR_FREQ_PENALTY:
                recent_action_frequency = np.mean(list(self.action_frequency_buffer))
                
                # --- START OF FIX #4: Adaptive Action Frequency Threshold ---
                vol_scaler = np.clip(market_volatility * 100, 0.5, 2.0)
                dynamic_action_threshold = np.clip(self.optimal_action_threshold * vol_scaler, 0.15, 0.45)
                # --- END OF FIX #4 ---

                if recent_action_frequency > dynamic_action_threshold:
                    freq_excess = recent_action_frequency - dynamic_action_threshold
                    # --- START OF FIX #5: Use quadratic scaling for hyperactivity penalty ---
                    frequency_penalty = (freq_excess ** 2) * self.weights.get('frequency_penalty', 0.0)
                    # --- END OF FIX #5 ---
                    intrinsic_components['frequency_penalty'] = frequency_penalty
                else:
                    intrinsic_components['frequency_penalty'] = 0.0
            else:
                # Not enough data yet to calculate a stable frequency penalty
                intrinsic_components['frequency_penalty'] = 0.0
            # --- END OF CRITICAL FIX (FIX #5) ---
            ...
            # --- START OF CRITICAL FIX (FIX #2): Decouple Rewards to Prevent Hacking ---
            self.reward_normalizer.update(raw_reward)
            normalized_raw_reward = self.reward_normalizer.normalize(raw_reward)
            total_reward = normalized_raw_reward + intrinsic_reward
            # --- END OF CRITICAL FIX (FIX #2) ---
...
class FixedHierarchicalTradingEnvironment(gymnasium.Env):
    ...
    def step(self, action: np.ndarray):
        ...
        try:
            ...
            # --- START OF CRITICAL FIX: Corrected Position Sizing & Entry Price Logic ---
            action_signal = np.clip(action[0], -1.0, 1.0)
            action_size = np.clip(action[1], 0.0, 1.0)

            action_magnitude = abs(action[0]) + abs(action[1])
            is_insignificant_action = False
            if action_magnitude > 0.01:
                self.total_attempted_trades += 1
                if action_size < 0.01:
                    self.total_insignificant_trades += 1
                    is_insignificant_action = True

            if action_size < 0.01:
                # If size is insignificant, we interpret this as a command to close the position.
                target_asset_quantity = 0.0
            else:
                # CRITICAL FIX #1: Decouple signal (for direction) from size (for magnitude)
                direction = np.sign(action_signal)
                
                # Calculate target notional based on direction and size independently
                ...
                effective_size_fraction = action_size * dynamic_limit
                target_notional = initial_portfolio_value * direction * effective_size_fraction
                
                target_asset_quantity = target_notional / current_price if current_price > 1e-8 else 0
            
            # Cap the position based on margin constraints
            ...
            # Calculate the actual trade based on the target
            ...
            # CRITICAL FIX #2: Implement Proper Entry Price Averaging Logic
            previous_asset_held = self.asset_held
            
            # Update balance with the PnL from the previous state and transaction costs
            self.balance += unrealized_pnl - total_cost
            
            # Correctly update the average entry price based on the trade
            if abs(trade_quantity) > 1e-8:
                # Case 1: Opening a new position from flat
                if previous_asset_held == 0:
                    self.entry_price = current_price
                # Case 2: Flipping position (e.g., long to short). Old position is closed, new one opened.
                elif np.sign(target_asset_quantity) != np.sign(previous_asset_held) and previous_asset_held != 0:
                    self.entry_price = current_price
                # Case 3: Adding to an existing position (e.g., buying more of a long)
                elif abs(target_asset_quantity) > abs(previous_asset_held):
                    total_cost_basis = (previous_asset_held * self.entry_price) + (trade_quantity * current_price)
                    self.entry_price = abs(total_cost_basis / target_asset_quantity)
                # Case 4: Reducing a position. The entry price of the remaining assets does NOT change.
                # No action is needed for self.entry_price in this case.
                
                self.trade_count += 1
            
            # Update the agent's holdings to the new target
            self.asset_held = target_asset_quantity
            
            # If the position is now fully closed, reset the entry price to zero for clarity.
            if abs(self.asset_held) < 1e-9:
            ...
            # Calculate previous notional for the reward calculator
            previous_notional = abs(previous_asset_held) * current_price
            ...
            reward, raw_reward, intrinsic_reward, reward_components = self.reward_calculator.calculate_immediate_reward(
                ...
                total_cost, is_insignificant_action, self.consecutive_inactive_steps,
    ...
    def _get_single_step_observation(self, step_index) -> dict:
        ...
        try:
            ...
            vol_percentile = self.risk_manager.get_volatility_percentile(self.volatility_estimate)
