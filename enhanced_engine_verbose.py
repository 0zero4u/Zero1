    ...
    def calculate_immediate_reward(self, prev_value: float, curr_value: float,
        ...
        try:
            ...
            if previous_action is not None:
                ...
                raw_components['transaction_penalty'] = tx_penalty * self.weights['transaction_penalty']
            else:
                raw_components['transaction_penalty'] = 0.0
...
class FixedHierarchicalTradingEnvironment(gymnasium.Env):
    ...
    def step(self, action: np.ndarray):
        ...
        try:
            ...
            # --- START OF FIX: Add a dead zone to prevent phantom "Size: 0" trades ---
            # If the requested size is negligible, we explicitly do nothing. This prevents
            # the bug where a position would change despite the log showing "Size: 0".
            if action_size < 0.01:
                # Interpret a near-zero size as a "hold" or "do nothing" action.
                # No change in position, no costs incurred.
                target_asset_quantity = self.asset_held
                trade_quantity = 0.0
                total_cost = 0.0
            else:
                # Original logic for executing a real trade
            ...
            # --- START OF FIX: Capture separated reward signals ---
            ...
            # --- START OF FIX: Add separated rewards to info dict ---
            info = {
    ...
    def reset(self, seed=None, options=None):
        """
        CORRECTED: Reset environment with a RANDOMIZED starting step to prevent overfitting
        and the 'Groundhog Day' bug.
        """
        try:
            ...
            # --- START OF FIX: Randomize the starting step ---
            warmup_period = self.cfg.get_required_warmup_period()
            
            # Define a minimum length for an episode to be meaningful
            min_episode_length = 5000  # e.g., at least 5000 steps
            
            # The latest possible starting point must allow for a full episode
            max_start_step = self.max_step - min_episode_length
            
            # Ensure the random range is valid
            if warmup_period >= max_start_step:
                # Fallback if the dataset is too short for randomization
                self.current_step = warmup_period
                if self.verbose:
                    logger.warning(f"Dataset too short for random start point sampling. Using fixed start: {self.current_step}")
            else:
                # Randomly select a starting step for this new episode
                # self.np_random comes from the parent Env class and handles seeding
                self.current_step = self.np_random.randint(warmup_period, max_start_step)
            # --- END OF FIX ---
