...
class FixedRewardCalculator:
    ...
    # --- END OF FIX ---
        ...
        try:
            ...
            # --- START OF MODIFICATION (Phase 2) ---
            # Calculate the un-normalized total reward
            unnormalized_reward = raw_reward + intrinsic_reward
            
            # Update the normalizer with the new un-normalized reward
            self.reward_normalizer.update(unnormalized_reward)
            
            # Get the final, normalized reward, replacing the old np.clip()
            total_reward = self.reward_normalizer.normalize(unnormalized_reward)
            # --- END OF MODIFICATION (Phase 2) ---
