...
class FixedHierarchicalTradingEnvironment(gymnasium.Env):
    ...
    def __init__(self, df_base_ohlc: pd.DataFrame, normalizer: Normalizer, config=None,
        ...
        try:
            ...
            for freq in all_required_freqs:
                # --- START OF FIX #4 ---
                # Added 'volume_delta': 'sum' to prevent it from being dropped during resampling.
    ...
    def step(self, action: np.ndarray):
        try:
            ...
            # 1. Calculate all reward components as before
            ...
            # 2. Isolate the raw market return (the only extrinsic, noisy component)
            base_return_component = reward_components.pop('base_return', 0.0)
            
            # 3. Update the normalizer ONLY with this noisy market signal
            self.reward_normalizer.update(base_return_component)
            
            # 4. Normalize ONLY the market signal
            normalized_return_reward = self.reward_normalizer.normalize(base_return_component)
            
            # 5. Sum all other intrinsic penalties/bonuses in their raw, unnormalized form
            intrinsic_reward = sum(reward_components.values())
            
            # 6. The final reward is the normalized market return PLUS the powerful, raw penalties
            reward = normalized_return_reward + intrinsic_reward
            
            # For logging purposes, raw_reward is now just the base return component
            raw_reward = base_return_component
    ...
    def _get_single_step_observation(self, step_index) -> dict:
        try:
            ...
            for key_enum, lookback in self.strat_cfg.lookback_periods.items():
                ...
                # The logic here was updated to correctly handle volume_delta
                if 'volume_delta' in key:
                    col_name = 'volume_delta'
                elif 'price' in key:
                    col_name = 'close'
                else: # Default for ohlc or others
                    col_name = 'open'

                # This line now correctly accesses 'volume_delta' when needed
            ...
            # --- START OF FIX #5 ---
            # Removed the incorrect '.value' call. Gymnasium Dict space keys are strings.
