...
class RewardManager:
    ...
    def __init__(self, weights: Dict[str, float]):
        ...
        # --- END OF REDESIGN ---
        # --- START OF FIX ---
        # Add a scaling factor to bring tiny PnL/cost ratios into a reasonable range
        # for the Z-score normalizer. 10,000 is a good starting point.
    ...
    def _transform_to_ratios(self, raw_components: Dict[str, float], **kwargs) -> Dict[str, float]:
        ...
        # --- START OF FIX: APPLY THE SCALING FACTOR ---
        ...
        # NOTE: Penalties below are already ratios or booleans, so they don't need scaling.
...
class FixedHierarchicalTradingEnvironment(gymnasium.Env):
    ...
    def _print_hold_info(self, step: int, price: float, portfolio_value: float, unrealized_pnl: float, action: np.ndarray):
        ...
        # Create a formatted string for the hold action
        log_message = (
    ...
    def step(self, action: np.ndarray):
        try:
            ...
            if is_executed_trade:
                ...
                # --- START OF FIX ---
                # This block was previously de-dented, causing a syntax error and making # subsequent functions unreachable. It is now correctly placed inside
                # the `if is_executed_trade:` block.
                if len(self.position_history_buffer) > 0:
                    last_position_sign = self.position_history_buffer[-1]
                    # A flip occurs if the sign changes and the previous position was not neutral (0).
            ...
            # --- START OF REDESIGN: Pass new values to the redesigned reward manager ---
            ...
            # --- START OF MODIFICATION ---
            # Replace the original if statement with this if/elif block
    ...
    def _get_single_step_observation(self, step_index) -> dict:
        try:
            ...
            for key_enum, lookback in self.strat_cfg.lookback_periods.items():
                ...
                ### START OF FIX 3: SIMPLIFIED TIMEFRAME PARSING ###
                freq = key.split('_')[-1]
                ### END OF FIX 3 ###
