...
class FixedRewardCalculator:
    ...
    # --- END OF FIX ---
        ...
        try:
            ...
            # --- 1. Calculate the PnL Reward (The "Truth Signal") ---
            # This is derived only from the net P&L of the step. This is the only signal we normalize.
            pnl_components = {}
            ...
            unnormalized_pnl_reward = normalized_return * self.weights['base_return']
            pnl_components['pnl_reward'] = unnormalized_pnl_reward
            
            # --- 2. Group All Risk and Behavior Penalties/Bonuses (The "Raw Tax") ---
            # These are applied directly to the final reward without normalization.
            risk_behavior_components = {}
            
            # --- Transaction Penalty ---
            normalized_cost = total_cost / max(prev_value, 1e-6)
            risk_behavior_components['transaction_penalty'] = normalized_cost * self.weights['transaction_penalty']
            
            # --- Drawdown Penalty ---
            current_drawdown = portfolio_state.get('drawdown', 0.0)
            if current_drawdown > 0.08:
                drawdown_excess = current_drawdown - 0.08
                penalty_factor = min(1.0, drawdown_excess / 0.12) ** 0.7
                risk_behavior_components['drawdown_penalty'] = penalty_factor * self.weights['drawdown_penalty']
            else:
                risk_behavior_components['drawdown_penalty'] = 0.0
            
            # --- Position Size Penalty ---
            position_size = abs(action[1])
            if position_size > 0.9:
                size_penalty = (position_size - 0.9) * 0.5
                risk_behavior_components['position_penalty'] = size_penalty * self.weights['position_penalty']
            else:
                risk_behavior_components['position_penalty'] = 0.0
            
            # --- Tiny Action Penalty (Thrashing Prevention) ---
            if 0 < trade_notional < (0.001 * prev_value):
                risk_behavior_components['tiny_action_penalty'] = self.weights.get('tiny_action_penalty', 0.0)
            else:
                risk_behavior_components['tiny_action_penalty'] = 0.0
            ...
            if inactivity_penalty_weight < 0 and consecutive_inactive_steps > self.inactivity_grace_period:
                ...
                ramp_progress = min(1.0, steps_into_penalty / self.penalty_ramp_up_steps) ** 0.5
                risk_behavior_components['inactivity_penalty'] = ramp_progress * inactivity_penalty_weight
            else:
                risk_behavior_components['inactivity_penalty'] = 0.0
            ...
            if position_size_action > 0.01:
                ...
                risk_behavior_components['exploration_bonus'] = min(0.1, exploration_reward) * self.weights['exploration_bonus']
            else:
                risk_behavior_components['exploration_bonus'] = 0.0
            
            # --- Frequency Penalty (Hyperactivity Prevention) ---
            action_magnitude = abs(action[0]) + abs(action[1])
            ...
            if len(self.action_frequency_buffer) == self.action_frequency_window:
                ...
                if recent_action_frequency > self.optimal_action_threshold:
                    ...
                    risk_behavior_components['frequency_penalty'] = frequency_penalty
                else:
                    risk_behavior_components['frequency_penalty'] = 0.0
            else:
                risk_behavior_components['frequency_penalty'] = 0.0

            # --- 3. Combine, Normalize, and Finalize ---
            components.update(pnl_components)
            components.update(risk_behavior_components)
            
            # Sum the raw "tax" from all behavior-shaping components
            risk_and_behavior_penalty = sum(risk_behavior_components.values())
            
            # Update the normalizer ONLY with the true P&L signal
            self.reward_normalizer.update(unnormalized_pnl_reward)
            
            # Get the normalized P&L reward
            normalized_pnl_reward = self.reward_normalizer.normalize(unnormalized_pnl_reward)
            
            # The final reward is the normalized P&L plus the raw penalty/bonus tax
            total_reward = normalized_pnl_reward + risk_and_behavior_penalty
            
            self.reward_history.append(total_reward)
            
            # For logging purposes, we'll report the normalized PnL as "raw_reward"
            # and the behavior penalty sum as "intrinsic_reward"
            return total_reward, normalized_pnl_reward, risk_and_behavior_penalty, components
...
class FixedHierarchicalTradingEnvironment(gymnasium.Env):
    ...
    def step(self, action: np.ndarray):
        ...
        try:
            ...
            reward, raw_reward, intrinsic_reward, reward_components = self.reward_calculator.calculate_immediate_reward(
                ...
                total_cost, trade_notional, self.consecutive_inactive_steps
            )
            # --- END OF FIX ---
    ...
    def _get_single_step_observation(self, step_index) -> dict:
        ...
        try:
            ...
            # --- START OF MODIFICATION (Phase 1) ---
            # Calculate the raw, unbiased volatility percentile.
            vol_percentile = self.risk_manager.get_volatility_percentile(self.volatility_estimate)
            
            # Create the new, more direct portfolio state vector of size 4.
            # We are replacing the biased 'regime_encoding' and 'vol_adjusted_signal'.
            raw_obs[FeatureKeys.PORTFOLIO_STATE.value] = np.array([
                normalized_position,
                normalized_pnl,
                margin_ratio,
                vol_percentile  # The new, unbiased feature for the Transformer to learn from
            ], dtype=np.float32)
            # --- END OF MODIFICATION (Phase 1) ---
