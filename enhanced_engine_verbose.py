...
class FixedHierarchicalTradingEnvironment(gymnasium.Env):
    ...
    def __init__(self, df_base_ohlc: pd.DataFrame, normalizer: Normalizer, config=None,
        ...
        try:
            ...
            default_reward_weights = {
                'realized_pnl': 3.0, 'unrealized_pnl_shaping': 0.1, 'trade_cost': 0.75,
                'drawdown': 1.5, 'thrashing': 2.0, 'frequency': 1.0, 'inactivity': 1.2,
                'action_clarity': 0.15,
            }
    ...
    def step(self, action: np.ndarray):
        try:
            ...
            # --- START OF TRADE COUNTER BUG FIX ---
            # The previous logic failed to count "closing" trades as "attempts",
            # leading to total_executed_trades > total_attempted_trades.
            # This new logic correctly defines an attempt.
            action_magnitude = abs(action_signal) * action_size
            
            is_opening_or_increasing = action_size >= 0.01
            is_closing = action_size < 0.01 and abs(self.asset_held) > 1e-8
            
            if is_opening_or_increasing or is_closing:
                self.total_attempted_trades += 1
            elif 0 < action_size < 0.01: # A tiny, insignificant action when flat
                self.total_insignificant_trades += 1
            # --- END OF TRADE COUNTER BUG FIX ---

            target_asset_quantity = 0.0
            if is_opening_or_increasing: # Use the same condition for consistency
            ...
            if is_executed_trade:
                ...
                if len(self.position_history_buffer) == 0 or self.position_history_buffer[-1] != current_position_sign:
