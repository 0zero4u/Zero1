...
class FixedRewardCalculator:
    ...
    # --- END OF FIX ---
        ...
        try:
            ...
            # --- Combine, Normalize, and Finalize ---
            
            # Combine all raw components into one dictionary
            all_raw_components = {**raw_components, **intrinsic_components}

            # Update history and stats for each component
            for component, value in all_raw_components.items():
                if component in self.reward_component_history:
                    ...
                    if len(history) > 50: # Start normalizing after 50 steps
            ...
            for component, value in all_raw_components.items():
                if component in self.reward_stats:
                    ...
                    normalized_components[component] = value # Pass through if no stats yet
            
            # Calculate final reward from NORMALIZED components
            total_reward_normalized = sum(normalized_components.values())
            
            # Clip the final normalized reward
            total_reward = np.clip(total_reward_normalized, -3.0, 3.0)
            
            # Return original raw sums for logging, but the new total_reward for learning
            raw_reward_unnormalized = sum(raw_components.values())
            intrinsic_reward_unnormalized = sum(intrinsic_components.values())
            
            self.reward_history.append(total_reward) # Keep history of final reward
            
            # For logging, return a dict with both raw and normalized values
            final_components_log = {**all_raw_components, **{f"{k}_norm": v for k, v in normalized_components.items()}}
