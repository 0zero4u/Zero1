...
class IndicatorConfig(BaseModel):
    ...
    expert_group: Literal['flow', 'volatility', 'value_trend', 'context'] = Field(description="The expert group this indicator's output feeds into")
...
class StrategyConfig(BaseModel):
    ...
    # NEW: Define context features as a single source of truth.
    context_feature_keys: List[str] = Field(
        default=[
            'bbw_1h_pct', 'price_dist_ma_4h', 'dist_vwap_3m',
            'dist_s1_3m', 'dist_r1_3m',
            'dist_s1_15m', 'dist_r1_15m', 'dist_s2_15m', 'dist_r2_15m',
            'dist_s1_1h', 'dist_r1_1h', 'dist_s2_1h', 'dist_r2_1h'
        ],
        description="List of context feature names, order must be preserved."
    )
    ...
    def lookback_periods(self) -> Dict[FeatureKeys, int]:
        ...
        The context lookback is now derived from the length of context_feature_keys.
        """
        return {
            ...
            FeatureKeys.CONTEXT: len(self.context_feature_keys),  # UPDATED: Dynamically set from the feature list
            FeatureKeys.VOLUME_DELTA_20S: 120, # NEW: Volume Delta feature
            FeatureKeys.VOLUME_DELTA_1M: 80,   # NEW: Volume Delta feature
        }
    ...
    class Config:
        # Pydantic v2 configuration for properties
        # This allows lookback_periods to be treated like a regular field
        # even though it's a @property.
        # This might not be strictly necessary depending on Pydantic version
        # but is good practice.
        orm_mode = True 
        allow_mutation = False # Since it's derived, don't allow it to be set directly
        
    @validator('lookback_periods', pre=True, always=True)
    def validate_lookback_periods(cls, v, values):
        # This validator now implicitly works with the dynamic property
        if not v: # v will be None initially because it's a property
             v = cls().lookback_periods # Get default values from a temp instance
...
class GlobalConfig(BaseModel):
    ...
    def get_required_warmup_period(self) -> int:
        ...
        try:
            ...
            vol_seconds = self.strategy.volatility_window * 3600  # On 1H data
            trend_seconds = self.strategy.trend_window * 4 * 3600  # On 4H data
            sr_seconds = self.strategy.support_resistance_window * 3600  # On up to 1H data
            ...
            for key, lookback in self.strategy.lookback_periods.items():
                if key == FeatureKeys.CONTEXT:
                    continue
                
                timeframe_str = key.value.split('_')[-1]
                timeframe_key = timeframe_str.upper().replace('M', 'T')
                
                if timeframe_key in timeframe_to_seconds:
                    seconds_needed = lookback * timeframe_to_seconds[timeframe_key]
            ...
            # Convert to number of base bars (20 seconds each)
            base_bar_seconds = 20  # Corresponds to TimeFrameType.SECOND_20
